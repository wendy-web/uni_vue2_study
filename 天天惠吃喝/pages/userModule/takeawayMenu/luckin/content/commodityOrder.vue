      ...
      <view class="order_box order_add" @click="changeShopHandle">
        <view class="fl_bet">
          ...
          <view class="remind_box fl_center">
            <image class="remind_icon" :src="takeImgUrl + '/lucky_gift.png'" mode="aspectFill"></image>
            到店取
          </view>
        </view>
        ...
        <view class="order_info-cont">
          <van-radio-group :value="radio" @change="onChangeHandle">
            <van-radio use-icon-slot
              ...
              :name="item.value"
            >
              <view slot="icon" :class="['clot_icon', radio === item.value  ? 'active' : '']"></view>
  ...
  methods: {
    ...
    async orderCreateRequest() {
      ...
      if(!this.isAloneProducts && !car_id) {
        car_id += this.selSubList.map(res => res.id) + ',';
      }
    ...
    createPayment() {
      ...
      uni.requestPayment({
        ...
        success: (res) => {
          this.$go( `/pages/userModule/takeawayMenu/luckin/order/index?oid=${this.oid}`);
          this.isShow = false;
        },
        fail: (res) => {
          this.$emit('cancelPay', {
