					...
					<view class="width-full display_row_between_center">
						...
						<view class="display_row_center">
							<div class="status-box">
								...
								<!-- <uv-tags text="已删除" type="info" plain v-if="item.status == -1"></uv-tags> -->
								<uv-tags text="待提审" type="primary" plain v-if="item.status == 0"></uv-tags>
								<uv-tags text="待验收" type="primary" plain v-else-if="item.status == 1"></uv-tags>
								<uv-tags text="已完成" type="success" plain v-else-if="item.status == 2"></uv-tags>
								<uv-tags text="已驳回" type="warning" plain v-else-if="item.status == 3"></uv-tags>
								<uv-tags text="已撤回" type="info" plain v-else-if="item.status == 4"></uv-tags>
								<uv-tags text="已作废" type="error" plain v-else-if="item.status == 5"></uv-tags>
							</div>
...
export default {
	...
	methods: {
		...
		async upCallback(page) {
			...
			const res = await getRepairListApi(params).catch(() => this.mescroll.endErr());
