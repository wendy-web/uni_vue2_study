<template>
	<view class="width-full contentBox position-r all-m-b-30 info-item">
		...
		<uv-picker ref="productPicker" :columns="[productOptions]" keyName="name" @confirm="confirmTypeHandle($event, 'product_line', productOptions)"></uv-picker>
		<!-- 时间的选择 -->
		<uv-datetime-picker
			ref="datetimePicker"
...
export default {
	...
	computed: {
		...
		productText() {
			return this.productOptions.find(res => res.id == this.formData.product_line)?.name;
		}
	},
	methods: {
		upDateForm(info) {
		...
		selTypeHandle(type) {
			...
			const setFindIndex = this.productOptions.findIndex(res => res.id == this.formData.product_line);
