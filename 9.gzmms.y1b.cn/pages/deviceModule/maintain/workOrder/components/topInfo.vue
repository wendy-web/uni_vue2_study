...
export default {
	...
	watch: {
		info: {
			...
			handler(newValue, oldValue) {
				if(!newValue) return;
				const { equipment_type_title, asset_no, bar_title, use_dept_names, spec, use_places, equipment } = newValue;
				this.formData = {
	...
	methods: {
		goToPropertyHandle() {
			uni.navigateTo({
				url: `/pages/deviceModule/maintain/repair/selDevice?deviceId=`
			})
		}
	},
};
