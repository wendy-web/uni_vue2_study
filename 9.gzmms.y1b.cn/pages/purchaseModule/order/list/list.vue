...
import { hasPerm, btnPermsMap } from "@/utils/auth.js";
export default {
	...
	methods: {
		...
		async tapStatus(e) {
			...
			const result = await getapproveLogApi({ document_type: 8, document_id: e.id });
		...
		tapVoid(e, item) {
			...
			uni.showModal({
				title: "温馨提示",
				content: `您确定要作废【${procure_no}】采购单吗?`,
				success: async (res) => {
					if (res.confirm) {
						console.log("用户点击确定");
		...
		handleAction(name, item) {
			...
			let { id, procure_no } = item;
			uni.showModal({
				title: "温馨提示",
				content: `您确定要删除【${procure_no}】的采购单吗`,
				success: async (res) => {
					if (res.confirm) {
						console.log("用户点击确定");
						let result = await delOrderApi({ id });
						this.toastRefresh(result.msg);
					} else if (res.cancel) {
						console.log("用户点击取消");
					}
	...
	computed: {
		checkDelBtn() {
			let sign = btnPermsMap.get(1)?.del || [];
			return hasPerm(sign);
		},
	},
	onHide() {}, // 生命周期 - 监听页面隐藏
