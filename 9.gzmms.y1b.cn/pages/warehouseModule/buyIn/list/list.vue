...
export default {
	...
	methods: {
		...
		async tapStatus(e) {
			...
			const result = await getapproveLogApi({ document_type: 1, document_id: e.id });
		...
		tapVoid(e, item) {
			...
			uni.showModal({
				title: "温馨提示",
				content: `您确定要作废【${wh_in_no}】采购入库单吗?`,
				success: async (res) => {
					if (res.confirm) {
						console.log("用户点击确定");
		...
		handleAction(name, item) {
			...
			uni.showModal({
				title: "温馨提示",
				content: `您确定要删除【${wh_in_no}】采购入库单吗?`,
				success: async (res) => {
					if (res.confirm) {
						console.log("用户点击确定");
						let result = await delBuyInApi({ id });
						this.toastRefresh(result.msg);
					} else if (res.cancel) {
						console.log("用户点击取消");
					}
