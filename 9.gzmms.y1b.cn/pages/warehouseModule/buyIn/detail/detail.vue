			...
			<view class="main">
				<wtabs-swiper>
					<template v-slot:detail>
						<view class="content">
							<wdetail-info :detailInfo="info" :type="4"></wdetail-info>
							<wgoods-list :type="4" :goods="info.goods"></wgoods-list>
						</view>
						<flow-vue :order_type="2" :order_id="order_id" :status="info.status" :whId="info.in_wh_id"></flow-vue>
					</template>
					<template v-slot:log>
						<!-- 单据日志 -->
...
import {
	...
	rejectBuyInApi,
	approveBuyInApi,
} from "@/api/modules/buyIn.js";
import myMixin from "@/mixin/index.js";
import detailMixin from "@/mixin/detail_mixin.js";
import flowVue from "@/pages/warehouseModule/otherIn/components/flow.vue"
export default {
	...
	methods: {
		async getData() {
			...
			const result = await detailBuyInApi({ id: this.order_id });
			this.skeletonLoading = false;
			console.log("采购入库详情res", result);
		...
		tapVoid() {
			...
			uni.showModal({
				title: "温馨提示",
				content: `您确定要作废该采购入库单吗?`,
				success: async (res) => {
					if (res.confirm) {
						console.log("用户点击确定");
						let result = await voidBuyInApi({ id });
						this.toastRefresh(result.msg);
					} else if (res.cancel) {
						console.log("用户点击取消");
					}
		...
		async tapRecall() {
			let id = this.order_id;
			const result = await recallBuyInApi({ id });
			this.toastRefresh(result.msg);
		},
		// 点击审核通过
		async tapApprove() {
			let id = this.order_id;
			const result = await approveBuyInApi({ id });
			this.toastRefresh(result.msg);
		},
		// 触发点击驳回
		tapReject() {
