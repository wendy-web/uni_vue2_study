			...
			<view class="main">
				<wtabs-swiper>
					<template v-slot:detail>
						<view class="content">
							<wdetail-info :detailInfo="info" :type="5"></wdetail-info>
							<wgoods-list :type="5" :goods="info.goods"></wgoods-list>
						</view>
					</template>
					<template v-slot:log>
						<!-- 单据日志 -->
...
export default {
	...
	methods: {
		async getData() {
			...
			const result = await detailRetGoodsApi({ id: this.order_id });
			this.skeletonLoading = false;
			console.log("退货出库单详情res", result);
