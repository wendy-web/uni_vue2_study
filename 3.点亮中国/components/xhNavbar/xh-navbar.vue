<template>
	<view class="xh-navber-box">
		...
		<view class="xh-navber" :class="{'fixed':fixed}"
			:style="{height:statusBarHeight+navBarHeight+'px',backgroundColor:navberColor}">
			<!-- 状态栏 -->
			...
			<view class="xh-navber-nav" :class="titleAlign"
				...
				<!-- 默认中间部分 -->
				<view v-if="title&&!isHome" @click="titleHandle" class="title-text">{{title}}</view>
				<!-- 自定义中间部分 -->
				<slot name="title" />
				<button open-type="share" class="share-btn" :style="{right:menuWidth + 10 + 'px'}" :data-name="'topShare'" v-if="shareImg"></button>
				<!-- 胶囊安全宽度 -->
				<view class="capsule" v-if="titleAlign === 'titleRight'" :style="{width:menuWidth+'px'}"></view>
				<!-- 左边tools -->
				<image v-if="leftImage" @click="leftClick" :class="['left-tools', leftImgPad ? 'leftImgPad_active': '']"
					:src="leftImage" :style="{height:navBarHeight/2+'px',width:navBarHeight/2+'px'}" />
				<!-- //首页	 -->
				<view v-if="isHome" class="left-tools left-tools-home"  @click="leftClick">
...
<style lang="scss">
	...
	.share-btn{
		...
		width: 35rpx;
		height: 35rpx;
		background: url("https://file.y1b.cn/public/img/bfzx/share@2x.png") no-repeat;
