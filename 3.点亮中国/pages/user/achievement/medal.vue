					...
					<view class="medal-list">
						<view class="medal-item" v-for="item in list0" :key="item.id" @click="showMedal(item)">
							<view class="medal-item-box mib-unlock">
								...
								<image
                                    v-if="item.status === 0"
                                    class="water waterAni"
                                    mode="heightFix"
									src="/static/home/water_black.png"
                                    :style="{bottom:item.propRate}"
                                >
							</view>
							<!-- 解锁进度 -->
...
export default {
    ...
    methods: {
        getShareImg() {
           return this.$refs.medalShareCard.getImageUrl();
        },
        getShareImgData() {
            return {...this.shareItemDataItem, ...this.shareImgData};
        },
        shareSelect(e) {
            const { id } = e.detail;
            switch(id) {
        ...
        initData(o) {
            ...
            //获取勋章信息
            const Api = getUserMedalDiff;
            Api(true).then((res) => {
                const { list_0, list_1, total } = res.data;
                // let num = Date.now();
                // list_0.forEach((item) => {
                //     item.image += "?" + num;
                // });
...
.medal {
    ...
    .water {
		...
		transform: translateX(-100%);
        &.waterAni {
            animation: waterAnimation 5s infinite linear alternate;
        }
    }
