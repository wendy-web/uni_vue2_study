			...
			async payment() { //发起支付
				try {
						...
						return uni.showModal({
							...
							success: (res) => {
								if (res.confirm) {
									//跳转
									this.$reLaunch({
										url: '/pages/personal/myCardBag/index?type=1&currTabs=0'
									});
								}
							}
						});
					...
					if (JSON.stringify(parmas.pull_arr) === '{}') {
						log.setFilterMsg('zm支付异常2');
						log.info('zmrequestPayment error==>' + JSON.stringify(parmas));
						log.info('zmcheckCardVolume==>' + JSON.stringify(checkCardVolume));
						log.error('zmcheckCardVolume==>' + JSON.stringify(message));
					}
				} catch (e) {
					if (e?.type === 'loaction') {
						this.$navigateBack({
					...
					success: (res) => {
						...
						getcardqr({
							...
							if (_res.code == 1 && _res.data.pay_time) {
								...
								this.$reLaunch({
									url: `/pages/zm/exchangeCode/index?codeData=${_playParams.qr_code}&isplay=1&type=1`
								});
								this.getCardTopCount(); //更新顶部状态
								log.setFilterMsg('zm支付成功');
								log.info('zmrequestPayment success==>' + JSON.stringify(res));
