...
<template>
	<view class="to-convert-item" @click="$emit('setCheckItem',config)">
		...
		<view class="to-convert-right">
			<!-- 25周年 -->
			<image class="icon-logo" :src="'../../../static/images/mcb_no_converted'+(config.prizeratetype+24)+'.png'"
				mode="aspectFill"></image>
			<!-- 26周年 -->
			<text class="tcr-top-text">{{CARDTITLES[config.prizeratetype-1]}}</text>
			...
			<view class="go-dui-box" v-on:click.stop="directExchange">
				<view class="go-dui" :class="'go-dui'+(config.prizeratetype+24)">
					去使用
				</view>
			</view>
			<!-- 右边背景 -->
			...
			<!-- 仅剩？天 -->
			<view class="one_more_day" v-if="config.open&&config.prizeratetype!==6">
