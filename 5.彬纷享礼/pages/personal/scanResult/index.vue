<!-- 扫码结果页 -->
<template>
<view :class="{'scan_result': true, 'active': !newUser }">
<mescroll-body
	id="mescrollBody"
	ref="mescrollRef"
	@init="mescrollInit"
	@down="goodsMixin_downCallback"
	:down="goodsMixin_downOption"
	:up="goodsMixin_upOption"
	@up="goodsMixin_upCallback"
>
<xh-navbar
	title="扫码结果"
	navberColor="#F5F6FA"
	titleAlign="titleCenter"
	leftImage="../../static/images/left_black_arrow.png"
	titleColor="#333"
	:leftCallBack="true"
	@leftCallBack="leftCallBackHandle"
/>
	<block v-if="newUser">
		<view :style="{ '--height': newUserDomHeight + 'px'}"
			:class="{'cont_box': true,
				'active': isBackAni ,
				'height_active': isBackAni
			}"
		>
			<view :class="['front_box', startAni ? 'active' : '']" @click="startAniHandle" >
	...
	<block v-else>
		...
		<view class="scan-result-body">
			...
			<view class="xy-text-list">
				<view class="xtl-item">积分奖励及积分活动由平台方提供，“{{data.prizeratetype == 4 ? '战马1元乐享' : '1元换购'}}”活动</view>
			...
			<easy-loadimage imageClass="srb-bg" mode="widthFix"
				image-src="https://file.y1b.cn/public/img/bfxn/202103/bfxn_scan_result_210501_2.png"
			></easy-loadimage>
		</view>
		<!--<view class="scan-result-body" v-if="type===2">
				icon
			<view class="srb-i">
				<easy-loadimage imageClass="srb-i-icon" mode="widthFix" :image-src="data.icon">
				</easy-loadimage>
			</view>
				tips
			<view class="tips-msg">
		...
		<view class="btn-tools">
			...
			<view class="btn-item animated tada infinite" @click="goShopMall">
				...
				<image class="btn-item-bg" mode="widthFix" src="../static/scanResult/bfxl_scan_reslut_06.png"></image>
		...
		<view class="banner-ad" v-if="isShowAd">
			<xh-official-account customClass="official-account-scan" eventName="followwoa_2"></xh-official-account>
			<ad unit-id="adunit-5ed880a587aa1a35"></ad>
			<view class="van-submit-bar__safe"></view>
		</view>
	</block>
	<!-- 天天卡券模块 -->
	<view id="scanResultBody">
		<block v-if="showListCont">
			<view class="card_title">
				<van-image width="260rpx" height="44rpx" src="/pages/personal/static/ttxl_card_title1.png" />
			</view>
			<!-- 商品列表 -->
...
<!-- 固定模块 - 暂时关闭列表 -->
<view v-if="goodsMixin_list.length"
	:class="['ttxl-sticky', isSticky ? 'active' : '']"
...
<!-- 积分翻倍 -->
<double-dialog ref="doubleDialog" />
</view>
</template>
<script>
	import { mapGetters } from 'vuex';
	import { getTtxlUser } from '@/api/homeApi.js';
	import { videofb } from "@/api/homeApi.js"
	import doubleDialog from "./doubleDialog.vue"
	import { RewardedVideoAd } from "@/utils/index.js"
	...
	export default {
		...
		onLoad(option) {
			...
			_RewardedVideoAd = RewardedVideoAd('adunit-0b4435b20332832d', (complete) => {
				...
				videofb({ fbid: this.data.fbid }).then(res => {
					if (res.code == 1) {
						let {
							num
						} = res.data;
		...
		computed: {
			...mapGetters(['isShowAd', 'scanResult', 'isLogin', 'newUser', 'losingIcon']),
			showListCont() {
				return (this.goodsMixin_list.length > 0) && this.isCancel;
			},
			// 积分兑换的标题头
			ttxlCardHeadTop() {
				return this.newUserDomHeight + uni.upx2px(200);
			}
		},
		methods: {
			goShopMall() {
				if (this.$ttxlUserPosition(this.popoverKey)) {
					const query = wx.createSelectorQuery()
					query.select('#goodList').boundingClientRect()
					query.selectViewport().scrollOffset()
					query.exec((res) => {
						this.mescroll.scrollTo(_scrollTop + res[0].top - uni.upx2px(76))
					})
					return;
				}
			},
			...
			async initDomHeight() {
				const domBoxRes = await this.warpRectDom('newUserId');
			...
			async goToNewUserHandle() {
				// 新人跳转
				const statues = await this.$ttxlUserPosition('code_newuser');
				if(statues) return;
				this.isCancel = true;
				setTimeout(() => {
			...
			startAniHandle() {
				// 直接跳转到首页 - code_ntry暂无配置
				this.$ttxlUserPosition('code_ntry');
			},
			warpRectDom(idName) {
