<template>
<view class="starbucks">
<image class="starbucks_bg-icon" :src="takeImgUrl + '/starbucks_bg-icon.png'" mode="'scaleToFill'"></image>
<mescroll-body
  :sticky="true"
  ref="mescrollRef"
  @init="mescrollInit"
...
>
  ...
  >
    <view class="search_box" slot="title">
      <view class="title_left">
        <image class="title_img" src="https://file.y1b.cn/store/1-0/231010/6524c72b5962f.png" mode="heightFix"></image>
      </view>
  ...
  <view class="add_cont fl" :style="{ height: mescrollHeight }">
    <van-loading
      size="36px" color="#006241" vertical
      class="add_cont-load"
      v-if="isLoading"
    >加载中...</van-loading>
...
import { starbucksStore, starbucksMenus } from '@/api/modules/takeawayMenu/starbucks.js';
...
export default {
  ...
  computed: {
    ...mapGetters(['cartComList', 'submitList', 'province_name', 'city_name', 'storeCode', 'brand_id', 'city_lat', 'city_lon', 'isAutoLogin']),
    mescrollHeight() {
  ...
  watch: {
    ...
    tabsList(newValue) {
      ...
      newValue.forEach((res, index) => {
        const produceIndex = res.products.findIndex(res => (res.id == this.shop_product_id) || ((res.productId instanceof Array) && res.productId.includes(this.shop_product_id.toString())));
        if(produceIndex >= 0) {
  ...
  async onShow() {
    ...
    if(this.isPayBack && this.oid) {
      this.$go( `/pages/userModule/takeawayMenu/starbucks/order/index?oid=${this.oid}`);
  ...
  methods: {
    ...
    async upCallback(page) {
        ...
        const params = {
            lon: this.city_lon || lng,
            lat: this.city_lat || lat,
            city_name: this.city_name || this.location_city,
            rote: this.rote,
            location_city: this.location_city,
            storeCode: this.storeCode,
        };
        const res = await starbucksStore(params);
        if(res.code != 1) {
        ...
        // 不存在餐厅id时
      if(!this.storeCode) {
        this.setStoreCode(code); // 门店id
      }
        starbucksMenus().then(res => {
            this.mescroll.endSuccess(0, false);
    ...
    toBuyHandle(){
        ...
        // 去结算 - 确认订单
        this.$refs.commodityCart.popupClose(false);
    ...
    closeContinuePayHandle(oid) {
      ...
      this.$go( `/pages/userModule/takeawayMenu/starbucks/order/index?oid=${oid}`);
    },
    payBackHandle(oid) {
      this.oid = oid;
      this.isPayBack = true;
    ...
    // 去搜索
    toSearchHandle() {
      this.$go( `/pages/userModule/takeawayMenu/starbucks/search`);
    },
    // 直接退回首页
    topCallBack() {
    ...
    selectShopHandle() {
      this.$go(`/pages/userModule/takeawayMenu/starbucks/selectShop/index?pathSource=${this.pathSource}`);
    },
    goSelectShopHandle() {
