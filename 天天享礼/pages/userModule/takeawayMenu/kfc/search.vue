...
>
    ...
    <view class="cart_box fl_center" v-if="isShowCart" @click="goMcDonald">
        <view class="buy_num fl_center">
            <image class="widHei" :src="takeImgUrl + '/kfc_car.png'" mode="widthFix"></image>
...
export default {
    ...
    methods: {
        async init() {
            const res = await kfcHistory({ tag: 1 });
        ...
        goMcDonald() {
            this.$reLaunch(`/pages/userModule/takeawayMenu/kfc/index?searchCartNum=${this.cartNum}`);
        },
        ...
        async searchBtnHandle(item) {
            ...
            const res = await kfcMenus(params);
