...
>
  ...
  >
    <view class="search_box" slot="title">
      <view class="title_left">
        <image class="title_img" src="https://file.y1b.cn/store/1-0/23830/64eed6f4df764.png" mode="heightFix"></image>
      </view>
  ...
  <!-- 主要内容的盒子 -->
  <view class="add_cont fl"
    :style="{
      height: mescrollHeight,
      '--padding': isShowComBuy ? '130rpx' : '0rpx'
    }"
  >
    <van-loading
      size="36px" color="#0022AB" vertical
      class="add_cont-load"
      v-if="isLoading"
    >加载中...</van-loading>
    <!-- <block v-else> -->
...
export default {
  ...
  computed: {
    ...mapGetters(['cartComList', 'resultList', 'brand_id', 'submitList', 'restaurant_id', 'province_name', 'city_name', 'isAutoLogin']),
    mescrollHeight() {
  ...
  methods: {
    ...
    closeContinuePayHandle(oid) {
      ...
      this.$refs.commodityOrder.popupClose();
      this.mescroll.resetUpScroll();
      this.$go( `/pages/userModule/takeawayMenu/luckin/order/index?oid=${oid}`);
    },
    warpRectDom(idName) {
    ...
    toSearchHandle() {
        if (!this.isAutoLogin) return this.$go('/pages/tabAbout/login/index');
        this.$go( `/pages/userModule/takeawayMenu/luckin/search`);
    },
    // 直接退回首页
    topCallBack() {
    ...
    selectShopHandle() {
      ...
      this.$go(`/pages/userModule/takeawayMenu/luckin/selectShop/index?pathSource=${this.pathSource}`);
    },
    goSelectShopHandle() {
