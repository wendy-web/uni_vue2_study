		...
		<view class="mc_cont">
			...
			<view class="mc_line fl_bet">
				<view class="mc_line-item">
					<image class="line_item-con" src="https://file.y1b.cn/store/1-0/24415/661c8c56cdead.png" mode="aspectFill"></image>
					<view>线上购买</view>
				</view>
				<image class="line_item-lab" src="https://file.y1b.cn/store/1-0/24415/661c8cc139e2b.png" mode="aspectFill"></image>
				<view class="mc_line-item">
					<image class="line_item-con" src="https://file.y1b.cn/store/1-0/24415/661c8c6da18c7.png" mode="aspectFill"></image>
					<view>用券点餐</view>
				</view>
				<image class="line_item-lab" src="https://file.y1b.cn/store/1-0/24415/661c8cc139e2b.png" mode="aspectFill"></image>
				<view class="mc_line-item">
					<image class="line_item-con" src="https://file.y1b.cn/store/1-0/24415/661c8c7e452dc.png" mode="aspectFill"></image>
					<view>门店取餐</view>
				</view>
			</view>
...
let _request = false;
	export default {
		...
		methods:{
			...
			async popupShow(id, type){
                this.setSelRedPacket(false);
                this.setSelNewPacket(false);
				if(!this.restaurantStore) this.initLocation(type);
				this.init(id);
			},
			...
			closePayHandle() {
				...
				// 跳转订单支付详情
                this.$go(`/pages/userModule/order/detail?id=${this.order_id}&source=${this.source}&backPath=2`);
			},
			continuePayHandle() {
				this.isShowContinuePay = false;
			},
						...
						query(params).then(res => {
							...
							if (code == 1) {
								...
								if(this.couponInfo.goods_type || this.isMcDonaldType) {
									uni.reLaunch({
										url: `/pages/userModule/order/detail?id=${this.order_id}&payment=${pay_amount}&source=${this.isMcDonaldType ? 'myCoupon' : ''}`
									});
									return;
								}
								uni.redirectTo({
