...
>
    ...
    <view class="order_rem box_fl" v-if="currentStatus == 2">
      <image class="status_rice" :src="takeImgUrl + '/kfc_rice.png'" mode="aspectFill"></image>
      餐品制作中，等待生成取餐码
    </view>
    <!-- 等待取餐 || 已完成 -->
    <!-- 订单的状态 -->
    <view class="status_box" v-if="currentStatus == 3 || currentStatus == 4">
      <view class="status_txt semicircle fl_center">
        <image class="status_txt-icon" :src=" takeImgUrl + '/kfc_active.png'" mode="widthFix"></image>
        {{ currentStatus == 3 ? '已生成取餐码，请前往门店取餐' : '订单已完成'}}
      </view>
      <view class="status_code fl_center" v-if="codes">
        <view class="status_code-text">{{ codes }}</view>
      </view>
        ...
        >
          <view class="fl_al_start">
            <view class="com_img fl_center">
              <image class="widHei" :src="item.imageUrl" mode="widthFix"></image>
            </view>
            <view class="list_txt">
            ...
            <view class="price_box">
              <view class="price_num">
                <text style="font-size: 24rpx">¥</text> {{item.originPrice}}
              </view>
