  ...
  <view class="order">
    ...
    <view class="order_cont">
      <view class="order_title">
        <image class="star_order-icon" :src="takeImgUrl + '/star_order-icon.png'" mode="'scaleToFill'"></image>
        确认订单
      </view>
      <!-- 门店信息 -->
      <view class="order_box order_add" @click="changeShopHandle">
        <view class="fl_bet">
          ...
          <view class="remind_box fl_center">
            <image class="remind_icon" :src="takeImgUrl +'/star_gift.png'" mode="aspectFill"></image>
            到店取
          </view>
        </view>
      ...
      <view class="order_box order_info">
        <view class="fl_bet">
          ...
          <view class="order_default">到店取餐(默认)</view>
        </view>
        <view class="phone_box fl_bet">
...
export default {
  ...
  methods: {
    ...
    popupShow({ products, isAloneProducts }) {
      ...
      orderSure2(params).then((res) => {
        if(res.code == 1) {
          this.isShow = true;
          this.config = res.data;
          const {coupon_amount, pay_amount, phone_number, product, orderItems, savings } = this.config;
    ...
    async orderCreateRequest() {
      ...
      const orderCreateParams = {
        ...
        customerMobile: this.phoneValue,
        savings: this.savings
      }
      this.orderCreateParams = orderCreateParams;
      const res =  await starbucksOrder(this.orderCreateParams);
