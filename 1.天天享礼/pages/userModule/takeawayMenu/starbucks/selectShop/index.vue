...
<view class="select">
  ...
  >
    <view class="list_box">
      ...
      >
        <image class="radio_active" :src="takeImgUrl +'/starbucks_active.png'" mode="aspectFill"></image>
...
export default {
    ...
    computed: {
      ...mapGetters(['brand_id', 'city_name', 'province_name', 'cartComList', 'storeCode', 'city_lat', 'city_lon']),
      fixedTop() {
    ...
    methods: {
      ...
      async upCallback(page) {
        ...
        const params = {
          lon: this.city_lon || this.longitude,
          lat: this.city_lat || this.latitude,
          page: page.num,
          size: page.size,
          keyword: this.inputValue,
          storeCode: this.storeCode
        };
        starbucksStore(params).then(res => {
          ...
          if(res.code == 1) {
            ...
            this.selIndex = this.shopList.findIndex(res => res.code == this.storeCode);
            return;
          }
      ...
      async selShopHandle(item, index) {
        ...
        const { code } = item;
        this.sel_storeCode = code;
        this.selIndex = index;
        this.$showLoading('加载中');
        this.isShowLoading = true;
        // 相同的店铺 - 后退
        if(this.storeCode == code) {
          this.$hideLoading();
          this.isShowLoading = false;
          this.$back();
          return;
        }
        // const res = await storesCk({ storeCode: code });
        // this.isShowLoading = false;
        // this.$hideLoading();
