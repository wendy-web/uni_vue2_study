...
<view class="item">
	...
	<view class="order" @click="jumpLinkHandle(item)">
		...
		<view
			class="order_cont"
			v-for="(orderItem, index) in item.kfc.items"
			:key="index"
		>
	...
	<view class="pay-info" @click="jumpLinkHandle(item)">
		<view class="total_amount" v-if="item.kfc.total_amount>3">共{{ item.kfc.total_amount }}件</view>
		<text class="pay-info_label">
			{{[2,3,4,5].includes(Number(item.status))?'实付':'应付'}}
		</text>
