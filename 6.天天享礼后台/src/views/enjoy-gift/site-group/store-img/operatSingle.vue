<template>
  ...
  >
    ...
    >
      ...
      <n-form-item label="显示时间" :disabled="modalType === 1">
        <n-time-picker
...
function show(operatType, data) {
  ...
  if (operatType !== 3) {
    http.getSingleImage({ id: data.id }).then((res) => {
      let { id, title, image, lx_type, popover_start, popover_over } = res.data
      model.value = { id, title, image, lx_type, popover_start, popover_over}
      /**图片预览 */
      if (image) {
        fileList.value = [
          {
