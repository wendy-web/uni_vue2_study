<template>
  <CommonPage show-footer title="轮播管理">
    <template #action>
  ...
  <operat-carousel ref="operatCarouselRef" @refresh="refresh" />
</template>
...
import operatCarousel from './operatCarousel.vue'
...
//优惠券操作
const operatCarouselRef = ref(null)
//提示展示
const message = useMessage()
/**查看 */
function lookCoupon(row) {
  operatCarouselRef.value.show(1, row)
}
/**编辑 */
function editCoupon(row) {
  operatCarouselRef.value.show(2, row)
}
/**新增活动 */
function handleAdd() {
  operatCarouselRef.value.show(3)
}
/** 自定义单元格 */
...
function removeCoupon(row) {
  dialog.warning({
    ...
    onPositiveClick: function () {
      http.deleteBanner({ banner_id: row.id }).then(function (res) {
        if (res.code == 1) {
