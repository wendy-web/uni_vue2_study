    ...
    <n-drawer-content :title="modalType === 1 ? '查看' : '修改'" closable>
      ...
      >
        ...
        <n-form-item label="奖品设置" path="award">
          <div>
            <n-button mb-10 type="info" :disabled="model.award.length >= 6 || modalType === 1" @click="addPrize">添加奖品 {{ model.award.length }}/6
            </n-button>
...
<script setup>
import { ref } from 'vue'
import { NImage, NButton, NSelect, NInputNumber, useMessage, useDialog } from 'naive-ui'
  ...
  },
  {
    ...
    render(row, index) {
      return h('div', [
        ...
        //           let index = model.value.award.findIndex(function (item) {
        //             return item.id === row.id
        //           })
        //           model.value.award.splice(index, 1)
        //         },
        //       })
        //     },
        //   },
...
function init() {
  http.getInfo({ task_id }).then((res) => {
    ...
    if (reward_rules.length > 0) {
      ...
      reward_rules = Array.from({ length: 6 }, (v, i) => ({ award_id: '', position: i + 1, prob: 0 }))
    }
