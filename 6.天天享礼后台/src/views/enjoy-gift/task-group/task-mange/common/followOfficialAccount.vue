<template>
  ...
  >
    ...
    >
      ...
      <n-form-item v-if="model.tag === 'FOLLOW_EMS_CNPL'" label="文章地址" path="article_url">
        <n-input v-model:value="model.article_url" :disabled="modalType === 1" />
      </n-form-item>
...
function show(operatType, data) {
  ...
  http.getInfo({ task_id: data.id }).then((res) => {
    let { id: task_id, name, title, subtitle, image, credits, describe, tag, type, article_url } = res.data
