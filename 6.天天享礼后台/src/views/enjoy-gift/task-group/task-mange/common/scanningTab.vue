<template>
  ...
  >
    ...
    >
      ...
      <n-form-item label="累计扫码" path="num">
        <n-input-group>
          <n-input-number
            ...
            :style="{ width: '150px' }"
          />
          <n-input-group-label>次/周</n-input-group-label>
        </n-input-group>
      </n-form-item>
      <n-form-item label="累计扫码奖励" path="credits">
        <n-input-group>
          <n-input-number
...
function show(operatType, data) {
  ...
  http.getInfo({ task_id: data.id }).then((res) => {
    let { id: task_id, tag, name, title, subtitle, image, num, credits, describe, type } = res.data
