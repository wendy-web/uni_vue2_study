        ...
        tooltip: {
          ...
          formatter (params) { // params的个数等于折线的条数（对多条折线数据进行加单位，就需要遍历）
            ...
            for (var i = 0, l = params.length; i < l; i++) {
              ...
              let circle = `<div style="display:flex;align-items: center;justify-content: space-between;"><div><i style="margin-right:4px;font-size:14px;color:${params[i].color}">o</i>`
              relVal += circle + params[i].seriesName + '：' + `</div><b>${params[i].value}</b></div>`
            }
            if(notArr[params[0].name]) {
              relVal += '<b>备注</b><br /><p style="font-size: 13px;">' + notArr[params[0].name] + '</p>'
            }
            return relVal
          }
        },
