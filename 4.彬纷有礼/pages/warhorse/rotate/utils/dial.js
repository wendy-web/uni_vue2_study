...
export default class Dial {
  constructor(pageContext, opts) {
    ...
    this.rotatebg = opts.rotatebg // 背景图片
    this.pointer = opts.pointer // 背景图片
  ...
  start() {
    ...
    if (isStart) return
    this.isStart = true
    let endAddAngle = (awardNumer - 1) * singleAngle + singleAngle / 2 + target //中奖角度
    // let endAddAngle = (awardNumer - 1) * singleAngle + singleAngle/2 + 270  //额定金额
    // let endAddAngle = (awardNumer - 1) * singleAngle + singleAngle/2 + 500  //翻倍奖励
    console.log("this.page.InnerAudio_one:", this.page.stopAduio);
    this.page.stopAduio(1);
    // this.page.InnerAudio_two.play();
