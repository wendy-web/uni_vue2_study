...
Component({
  ...
  data: {
    ...
    dateRange: JSON.parse(JSON.stringify(initDateRange)), //今日 近7天 15 30
    pickerConfig, // 日期时分秒组件默认配置
    isPickerShow: false, // 显示日期时分秒组件
    selectDate: JSON.parse(JSON.stringify(initSelectDate)), // 开始时间、结束时间参数
    dateRangeName: 'dateRange',
    selectDateName: 'selectDate', //默认存储
    couponTypeName: 'couponTypeArr',
    goodTypeArrName: 'goodTypeArr',
    crewArrName: 'crewArr', //相关人员 默认存储名
    couponTypeArr: JSON.parse(JSON.stringify(initCouponType)), //卡券类型 只有筛选换购券的时候才有
    file_url_9m: `${COS_URL}/public/img/bfyl/2023/09`, //23年9月 cos图片资源
    // 折扣券、活动券相关数据初始化
    goodTypeArr: [], // 产品类型初始化 一共有5个
    crewArr: [],
  },
  ...
  methods: {
    ...
    showFilterPop() {
      ...
      let dateRangeName = 'dateRange';
      let selectDateName = 'selectDate';
      let couponTypeName = 'couponTypeArr';
      let goodTypeArrName = 'goodTypeArr';
      let crewArrName = 'crewArr';
    ...
    resetFilter(event) {
      ...
      // 告诉主页重置 品牌的类型
      this.triggerEvent('resetBrandType')
      this.setData({
    ...
    initSkuParams(skuParams) {
      ...
      // 红牛、战马产品类型初始化 
      let goodTypeArr = JSON.parse(JSON.stringify(skuParams));
      this.setData({
        goodTypeArr,
      })
    },
    initMember(member) {
      ...
      // 红牛、战马产品类型初始化 
      let crewArr = JSON.parse(JSON.stringify(member));
      this.setData({
        crewArr,
      })
    },
    // 页面tab 选中的筛选条件，替换或者重置 弹窗条件
    replaceGoodTypeItem(item){
      let {arg,checked} = item;
      let {goodTypeArr} = this.data;
      goodTypeArr.forEach(item=>{
        item.set.forEach(itm=>{
          if(itm.arg == arg){
            itm.checked = checked
