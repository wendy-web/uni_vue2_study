// pages/phaseIII/useCoupon/component/hxing/index.js
...
Component({
  ...
  methods: {
        ...
        if (filterArr.length) {
          filterArr.forEach(item => {
            ...
            if (item.name.search('crewArr_') > -1) {
              params.apply_uid = item.check_uid
            }
          })
        }
    ...
    async loadmore(event) {
      ...
      try {
        ...
        console.log("核销中 loadmore catch:", error)
      }
    ...
    initFilterData(event) {
      console.log("核销中  列表收到了筛选条件：", event)
      let {
        filterArr = []
      } = event;
  ...
  lifetimes: {
    ...
    attached() {
      ...
      query.exec(res => {
        console.log("hxing bounding:", res)
        let item = res[0] || '';
