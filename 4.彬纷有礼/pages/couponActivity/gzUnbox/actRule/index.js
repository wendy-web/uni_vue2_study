// pages/couponActivity/gzUnbox/actRule/index.js
const utils = require("../../../../utils/util");

import shopActUtils from '../../../../api/shopActivity/utils';
import {unboxInitPrizeConfig} from '../../config/index';

let app = getApp();
let COS_URL = app.globalData.COS_URL;
Page({

  /**
   * 页面的初始数据
   */
  data: {
    bg_num: 'data:image/png;base64,',
    img_rule_title: `${COS_URL}/public/img/bfyl/2023/07/unbox/img_act_rule.png`,
    img_rule_unbox: `${COS_URL}/public/img/bfyl/2023/07/unbox/img_rule_unbox.png`,
    icon_arrow_up: `${COS_URL}/public/img/bfyl/2023/07/unbox/icon_arrow_up.png`,
    prizeConfig: null, //奖项配置，接口返回
    file_url_gzunbox:`${COS_URL}/public/img/bfyl/2023/08/gzUnbox`,//广州开箱活动资源
  },

  /**
   * 生命周期函数--监听页面加载
   */
  onLoad(options) {
    utils.getNavbarData().then(res => {
      if (res) {
        this.setData({
          navBarSystem: res,
        })
      }
    });
    this.initOptions(options)
  },

  /**
   * 生命周期函数--监听页面初次渲染完成
   */
  onReady() {

  },

  /**
   * 生命周期函数--监听页面显示
   */
  onShow() {

  },
  async initOptions(options) {
    console.log("initOptions:", options)
    let {
      actInfo
    } = options;
    if (actInfo) {
      actInfo = JSON.parse(decodeURIComponent(actInfo))
      this.setData({
        actInfo,
        prizeConfig: unboxInitPrizeConfig(actInfo.calc.rule)

      })
    } else {
      let {
        actInfo
      } = await shopActUtils.gzUnboxActInfo();
      this.setData({
        actInfo,
        prizeConfig: unboxInitPrizeConfig(actInfo.calc.rule)
      })
    }
  },

  /**
   * 生命周期函数--监听页面隐藏
   */
  onHide() {

  },

  /**
   * 生命周期函数--监听页面卸载
   */
  onUnload() {

  },

  /**
   * 页面相关事件处理函数--监听用户下拉动作
   */
  onPullDownRefresh() {

  },

  /**
   * 页面上拉触底事件的处理函数
   */
  onReachBottom() {

  },
  // 返回上一页
  back: function () {
    wx.navigateBack({
      delta: 0,
    })
  },
  // 返回顶部
  scrollTop: function () {
    wx.pageScrollTo({
      scrollTop: 0,
      duration: 300,
    })
  },
 
})