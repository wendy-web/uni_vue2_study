	...
	<view class="uv-calendar">
		<view class="uv-calendar__content" v-if="insert">
			<calendar-body
				...
				@next="next"
				@backToday="backToday"
				@selRangDay="selRangDay"
				@selRangWeek="selRangWeek"
				@selRangMonth="selRangMonth"
				@selRangNextMonth="selRangNextMonth"
	...
	export default {
		...
		methods: {
			...
			selRangDay() {
				...
				const date = this.cale.getDate(new Date())
				const todayYearMonth = `${date.year}-${date.month}`;
				this.init([date.fullDate, date.fullDate])
				if (nowYearMonth !== todayYearMonth) {
					this.monthSwitch()
				}
				this.change()
			...
			selRangWeek(){
				const nowYearMonth = `${this.nowDate.year}-${this.nowDate.month}`
				const date = this.cale.getDate(new Date())
				const currentDate = this.cale.getCurrentDate(new Date())
				const todayYearMonth = `${date.year}-${date.month}`;
				console.log('currentDate', currentDate)
				this.init([currentDate.weekStart, currentDate.weekEnd])
				if (nowYearMonth !== todayYearMonth) {
					this.monthSwitch()
				}
				this.change()
			...
			selRangMonth() {
				...
				const currentDate = this.cale.getCurrentDate(new Date())
				const todayYearMonth = `${date.year}-${date.month}`;
				this.init([currentDate.monthStart, currentDate.monthEnd])
				if (nowYearMonth !== todayYearMonth) {
					this.monthSwitch()
				}
				this.change()
			...
			selRangNextMonth() {
				...
				const currentDate = this.cale.getCurrentDate(new Date())
				const todayYearMonth = `${date.year}-${date.month}`;
				this.init([currentDate.nextMonthStart, currentDate.nextMonthEnd])
				if (nowYearMonth !== todayYearMonth) {
					this.monthSwitch()
				}
				this.change()
