<template>
	<view class="uv-calendar-body">
		...
		<view class="uv-calendar__header" v-if="showFastBtn && range">
			<view class="uv-calendar__header-Item" @click="selRangDay">{{todayText}}</view>
			<view class="uv-calendar__header-Item" @click="selRangWeek">{{toWeekText}}</view>
			<view class="uv-calendar__header-Item" @click="selRangMonth">{{toMonthText}}</view>
			<view class="uv-calendar__header-Item" @click="selRangNextMonth">{{ toNextMonthText }}</view>
		</view>
...
import i18nMessages from './i18n/index.js';
	...
	export default {
		...
		computed: {
			...
			toWeekText() {
				return t("uv-calender.toWeek")
			},
			toMonthText() {
				return t("uv-calender.toMonth")
			},
			toNextMonthText() {
				return t("uv-calender.toNextMonth")
			},
			monText() {
		...
		methods: {
			...
			selRangDay() {
				this.$emit('selRangDay');
			},
			selRangWeek() {
				this.$emit('selRangWeek');
			},
			selRangMonth() {
				this.$emit('selRangMonth');
			},
			selRangNextMonth() {
				this.$emit('selRangNextMonth');
			},
			pre() {
				this.$emit('pre');
			},
