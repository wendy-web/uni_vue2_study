		...
		>
			<slot name="icon">
				...
				<view class="uv-steps-item__wrapper__icon" v-else-if="parentData.activeIcon || parentData.inactiveIcon">
					...
					></uv-icon> -->
					<uv-icon
						:name="success ? parentData.activeIcon : parentData.inactiveIcon"
						:size="iconSize"
						:color="success ? parentData.activeColor : parentData.inactiveColor"
					></uv-icon>
				</view>
				<view
					v-else
		...
		<view :class="['uv-steps-item__content', `uv-steps-item__content--${parentData.direction}`]" :style="[contentStyle]">
			<view class="flex-row">
				<slot name="title">
					...
					></uv-text> -->
					<uv-text :text="title" :type="success || reverse ? 'main' : 'content'" lineHeight="20px" :size="success ? 14 : 13"></uv-text>
				</slot>
				<text class="right-title">{{ rightTitle }}</text>
			</view>
			<view class="flex">
				<slot name="desc">
					<uv-text :text="desc" :type="success || reverse ? 'main' : 'content'" size="12"></uv-text>
				</slot>
				<text class="right-desc">{{ rightDesc }}</text>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	computed: {
		...
		statusClass() {
			const { index, error, success } = this;
