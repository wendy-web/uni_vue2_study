	...
	<view class="tree">
		...
		<view class="tree-cnt" :class="{ show: showTree }">
			<view class="tree-bar">
				...
				<!-- <view class="tree-bar-confirm" :style="{ color: confirmColor }" hover-class="hover-c" @tap="_confirm">确定</view> -->
				<view class="tree-bar-confirm" :style="{ color: confirmBtnColor }" hover-class="hover-c" @tap="_confirm">
					确定
				</view>
			</view>
				...
				<scroll-view class="tree-view-sc" :scroll-y="true">
					...
					>
						...
						>
							...
							>
								<view
									class="tree-check-yes"
									v-if="item.checked"
									:class="{ radio: !multiple }"
									:style="{ 'border-color': confirmColor }"
								>
									<view class="tree-check-yes-b" :style="{ 'background-color': confirmColor }"></view>
								</view>
								<view
									class="tree-check-no"
									v-else
...
export default {
	...
	props: {
		...
		showParentName:{
			//选中子集时,显示父级的name,默认false
			type: Boolean,
			default: false,
		},
		showParentId:{
			//选中子集时,显示父级的id,默认false
			type: Boolean,
			default: false,
		},
		idKey: {
	...
	methods: {
		...
		_cancel() {
			...
			if (this.confirmList.length === 0) this._reTreeList();
			this._hide();
			this.$emit("cancel", []);
		},
		_confirm() {
			...
			this.treeList.forEach((v, i) => {
				if (this.treeList[i].checked) {
					// this.getParentLabel(v);
					rt.push({
						id: this.showParentId ? this.getParentId(v) : this.treeList[i].id,
						// name: this.treeList[i].name,
						name: this.showParentName ?  this.getParentLabel(v) : this.treeList[i].name,
						value: this.treeList[i].value,
					});
				}
			});
		...
		getParentId(item) {
			...
			let parentList = item.parentId;
			let list = this.treeList.filter((el) => {
				return parentList.includes(el.id);
			});
			list.sort((a, b) => a.level - b.level);
			let idList = list.map((el) => el.id);
			let idStr = idList.join(",");
			// console.log("nameStr", nameStr);
			if(idStr) idStr += ',';
			return  idStr + item.id;
		},
		getParentLabel(item) {
			...
			list.sort((a, b) => a.level - b.level);
			let nameList = list.map((el) => el.name);
		...
		_initShowTree() {
			...
			if (this.cascade && this.multiple && this.selectParent) {
				...
				for (i; i >= 0; i--) {
					//获取父级下标
					const nextIndex = treeList.findIndex((item) => item.id == checkedParentSetList[i]);
					//从父级数据中取到所有同级id数组
					const sonarrId = treeList[nextIndex]?.sonarrId;
					if (sonarrId.length > 0 && this.getInclude(checkedSetList, sonarrId)) {
						treeList[nextIndex].checked = true;
						checkedSetList = [...checkedSetList, treeList[nextIndex].id];
					}
				}
			}
			const newTreeList = treeList.map((treeItem) => {
				...
				// 兄弟也展开 也显示
				if (this.getInclude(checkedParentSetList, treeItem.parentId)) {
					treeItem.show = true;
				}
				// 所有checked的 都选中
