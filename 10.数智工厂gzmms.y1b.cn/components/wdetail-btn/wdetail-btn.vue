<template>
	<view class="bottom-btn">
		...
		<template v-if="checkAssocTypeFn(5)">
			<!-- 采购入库单、其他入库单 -->
			<template v-if="status == 7 && [4, 12].includes(type)">
				<view class="btn-item" @click="tapWhApprove" v-if="checkBtn(whapproveSign)">
					<image src="https://file.y1b.cn/public/erpxcx_img/common/tongguo@2x.png" mode="" class="btn-item-img"></image>
					<text>入库确认</text>
				</view>
				<view class="btn-item" @click="tapWhReject" v-if="checkBtn(whrejectSign)">
					<image src="https://file.y1b.cn/public/erpxcx_img/common/bohui@2x.png" mode="" class="btn-item-img"></image>
					<text>入库驳回</text>
				</view>
			</template>
			<!-- 退料入库单 -->
			<template v-if="status == 8 && [7].includes(type)">
				<view class="btn-item" @click="tapWhApprove" v-if="checkBtn(whapproveSign)">
					<image src="https://file.y1b.cn/public/erpxcx_img/common/tongguo@2x.png" mode="" class="btn-item-img"></image>
					<text>入库确认</text>
				</view>
			...
			<!-- 调拨单 -->
			<template v-if="status == 8 && [9].includes(type)">
				<view class="btn-item" @click="tapWhInOutApprove" v-if="checkBtn(whapproveSign)">
					<image src="https://file.y1b.cn/public/erpxcx_img/common/tongguo@2x.png" mode="" class="btn-item-img"></image>
					<text>{{ type9InOutStatus == 1 ? "调出确认" : "调入确认" }}</text>
				</view>
				<view class="btn-item" @click="tapWhReject" v-if="checkBtn(whrejectSign)">
					<image src="https://file.y1b.cn/public/erpxcx_img/common/bohui@2x.png" mode="" class="btn-item-img"></image>
					<text>{{ type9InOutStatus == 1 ? "调出驳回" : "调入驳回" }}</text>
				</view>
			</template>
		</template>
		<template v-if="checkAssocTypeFn(8)">
			<!-- 调拨单 -->
			<template v-if="status == 9 && [9].includes(type)">
				<view class="btn-item" @click="tapWhInOutApprove" v-if="checkBtn(whapproveSign)">
					<image src="https://file.y1b.cn/public/erpxcx_img/common/tongguo@2x.png" mode="" class="btn-item-img"></image>
					<text>{{ type9InOutStatus == 1 ? "调出确认" : "调入确认" }}</text>
				</view>
...
export default {
	...
	methods: {
		...
		// 入库确认
		tapWhApprove() {
			this.$emit("tapWhApprove");
		},
		tapWhInOutApprove() {
			this.$emit("tapWhInOutApprove", { status: this.type9InOutStatus });
		},
		// 入库驳回
		tapWhReject() {
			this.$emit("tapWhReject");
		},
	},
};
</script>
