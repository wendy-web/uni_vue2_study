			...
			<view class="top">
				...
				<view class="order-status" @click="tapStatus">
					...
					<template v-else-if="info.status == 8">
						<!-- 调拨单 -->
						<uv-text type="warning" text="待调出确认" bold size="14" v-if="type == 9"></uv-text>
						<!-- 退料入库单 -->
						<uv-text type="warning" text="待退料" bold size="14" v-else-if="type == 7"></uv-text>
						<!--  -->
						<uv-text type="warning" text="待领料" bold size="14" v-else></uv-text>
					</template>
			...
			<slot name="operation">
				...
				<template v-if="checkAssocTypeFn(1)">
					...
					<template v-if="info.status == 0 || info.status == 4 || info.status == 5">
						...
						<view class="footer-btn" v-if="checkBtn(submitSign)">
							<uv-button
								text="提审"
								shape="circle"
								color="#6086fc"
				...
				<!-- 当前为仓库待确认的时候,即为仓库确认人 -->
				<template v-if="checkAssocTypeFn(5)">
					<!-- 采购入库单、其他入库单 -->
					<template v-if="info.status == 7 && [4, 12].includes(type)">
						<view class="footer-btn" v-if="checkBtn(whapproveSign)">
							<uv-button
								text="入库确认"
								shape="circle"
								color="#6086fc"
						...
						<view class="footer-btn" v-if="checkBtn(whrejectSign)">
							<uv-button
								text="入库驳回"
								shape="circle"
								color="#f9ae3d"
					...
					<!-- 退料入库单-->
					<template v-if="info.status == 8 && [7].includes(type)">
						<view class="footer-btn" v-if="checkBtn(whapproveSign)">
							<uv-button
								text="入库确认"
								shape="circle"
					...
					<!-- 调拨单 -->
					<template v-if="info.status == 8 && [9].includes(type)">
						<view class="footer-btn" v-if="checkBtn(whapproveSign)">
							<uv-button
								:text="type9InOutStatus == 1 ? '调出确认' : '调入确认'"
								shape="circle"
								color="#6086fc"
								:customStyle="footerBtn"
								@click="cellwhInOutApprove"
							></uv-button>
						</view>
						<view class="footer-btn" v-if="checkBtn(whrejectSign)">
							<uv-button
								:text="type9InOutStatus == 1 ? '调出驳回' : '调入驳回'"
								shape="circle"
								color="#f9ae3d"
								:customStyle="footerBtn"
								@click="cellwhReject"
				...
				<template v-if="checkAssocTypeFn(8)">
					<!-- 调拨单 -->
					<template v-if="info.status == 9 && [9].includes(type)">
						<view class="footer-btn" v-if="checkBtn(whapproveSign)">
							<uv-button
								:text="type9InOutStatus == 1 ? '调出确认' : '调入确认'"
								shape="circle"
				...
				<view class="footer-btn" v-if="checkBtn(detailSign)">
...
export default {
	...
	computed: {
		...
		// 入库确认
		whapproveSign() {
			let sign = btnPermsMap.get(this.type)?.whapprove || [];
			return sign;
		},
		// 入库反驳
		whrejectSign() {
			let sign = btnPermsMap.get(this.type)?.whreject || [];
			return sign;
		},
	...
	methods: {
		...
		cellwhApprove() {
			...
			this.$emit("tapWhApprove", { id, in_time: formartDate(in_time) });
		},
		cellwhInOutApprove() {
			let { id, in_time, out_time } = this.info;
			this.$emit("tapWhInOutApprove", {
				id,
				in_time: formartDate(in_time),
				out_time: formartDate(out_time),
				status: this.type9InOutStatus,
			});
		},
		// 点击仓库驳回按钮
		cellwhReject() {
