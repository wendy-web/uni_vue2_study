			...
			<view class="main">
				<wtabs-swiper>
					<template v-slot:detail>
						<view class="content">
							<wdetail-info :detailInfo="info" :type="2"></wdetail-info>
							<wgoods-list :type="2" :goods="info.goods"></wgoods-list>
						</view>
					</template>
					<template v-slot:log>
						<!-- 单据日志 -->
...
export default {
	...
	methods: {
		async getData() {
			...
			const result = await detailRefundApi({ id: this.order_id });
			this.skeletonLoading = false;
			console.log("采购退货单详情res", result);
