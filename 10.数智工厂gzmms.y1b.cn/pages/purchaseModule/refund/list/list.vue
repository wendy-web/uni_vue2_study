...
export default {
	...
	methods: {
		...
		async tapStatus(e) {
			...
			const result = await getapproveLogApi({ document_type: 9, document_id: e.id });
		...
		tapVoid(e, item) {
			// uni.$uv.toast("点击作废");
			let id = e.id;
			let { procure_ret_no } = item;
			uni.showModal({
				title: "温馨提示",
				content: `您确定要作废【${procure_ret_no}】采购退货单吗?`,
				success: async (res) => {
					if (res.confirm) {
						console.log("用户点击确定");
						let result = await voidRefundApi({ id });
						this.toastRefresh(result.msg);
					} else if (res.cancel) {
						console.log("用户点击取消");
					}
		...
		async tapRecall(e) {
			let id = e.id;
			const result = await recallRefundApi({ id });
			this.toastRefresh(result.msg);
		},
		// 触发点击通过
		async tapApprove(e) {
			let id = e.id;
			const result = await approveRefundApi({ id });
			this.toastRefresh(result.msg);
		},
		// 触发点击驳回
		tapReject(e) {
		...
		handleAction(name, item) {
			...
			let { id, procure_ret_no } = item;
			uni.showModal({
				title: "温馨提示",
				content: `您确定要删除【${procure_ret_no}】的采购退货单吗`,
				success: async (res) => {
					if (res.confirm) {
						console.log("用户点击确定");
						let result = await delRefundApi({ id });
						this.toastRefresh(result.msg);
					} else if (res.cancel) {
						console.log("用户点击取消");
					}
	...
	computed: {
		checkDelBtn() {
			let sign = btnPermsMap.get(2)?.del || [];
			return hasPerm(sign);
		},
	},
	onHide() {}, // 生命周期 - 监听页面隐藏
