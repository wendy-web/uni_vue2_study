<template>
	<view class="width-full all-p-tb-30 all-p-lr-20">
		<view class="contentBox error_box" v-if="!scanInfo.asset_no" >
			<image class="error_icon" src="/static/home/u12383.svg" mode="widthFix"></image>
			<view class="error_cont">
				<view class="t-c-6F6F6F" >扫码结果</view>
				<view class="t-c-000018 f-s-42 t-w-bold all-m-t-30">{{ content }}</view>
				<view class="t-c-6F6F6F f-s-24 all-m-t-30">该编码未登记在册，请联系管理员核实二维码信息</view>
			</view>
		</view>
		<view class="width-full contentBox position-r all-m-b-30" v-else>
...
import { getEquipmentScanNotTokenApi} from "@/api/device/common/index.js";
...
export default {
	data() {
		return {
			menuList: [
				{
					name: '设备维修',
					img: '/static/deviceHome/equipment.png',
					// num: 1,
					path: '/pages/deviceModule/maintain/repair/list?authBack=true&',
				},
				{
					name: '保养计划',
					img: '/static/deviceHome/maintain-plan.png',
					// num: 99,
					path: '/pages/deviceModule/maintain/plan/list?authBack=true&'
				},
				{
					name: '保养工单',
					img: '/static/deviceHome/maintain-workOrder.png',
					path: '/pages/deviceModule/maintain/workOrder/list?noValidationLogin=true&'
				},
				{
					name: "点巡检计划",
					img: "/static/deviceHome/plan.png",
					path: "/pages/deviceModule/inspection/plan/list?authBack=true&",
				},
				{
					name: "点巡检记录",
					img: "/static/deviceHome/record.png",
					path: "/pages/deviceModule/inspection/record/list?noValidationLogin=true&",
				},
				// {
				// 	name: '抄表记录',
	...
	// 生命周期 - 监听页面加载
	onLoad(options) {
		// q=https%253A%252F%252Fgzmms.y1b.cn%252Fbjqr%253Fc%253D15-16-2&scancode_time=1735111263
	...
	methods: {
		...
		...mapActions({
			getPermsList: "user/getPermsList",
		}),
		async getData() {
			try {
				const result = await getEquipmentScanNotTokenApi({ content: this.content });
				this.loading = false;
				this.scanInfo = result.data;
				this.moduleType = result.data?.module_type || 0;
				this.SETMODULETYPE(this.moduleType);
				this.token && this.getPermsList();
			} catch(error) {
		...
		targetPage(url) {
			this.SETMODULETYPE(this.moduleType);
			let { asset_no, id } = this.scanInfo;
			if (url) {
				uni.navigateTo({
					url: `${url}asset_no=${asset_no}&eq_id=${id}`,
				});
			}
		},
	},
