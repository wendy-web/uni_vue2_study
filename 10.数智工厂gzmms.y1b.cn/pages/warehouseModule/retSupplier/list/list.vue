...
export default {
	...
	methods: {
		...
		async tapStatus(e) {
			...
			const result = await getapproveLogApi({ document_type: 4, document_id: e.id });
		...
		async tapVoid(e, item) {
			// uni.$uv.toast("点击作废");
			let id = e.id;
			let { wh_recin_no } = item;
			uni.showModal({
				title: "温馨提示",
				content: `您确定要作废【${wh_recin_no}】退料入库单吗?`,
				success: async (res) => {
					if (res.confirm) {
						console.log("用户点击确定");
		...
		handleAction(name, item) {
			...
			let { id, wh_recin_no } = item;
			uni.showModal({
				title: "温馨提示",
				content: `您确定要删除【${wh_recin_no}】退料入库单吗?`,
				success: async (res) => {
					if (res.confirm) {
						console.log("用户点击确定");
						let result = await delRetSupInApi({ id });
						this.toastRefresh(result.msg);
					} else if (res.cancel) {
						console.log("用户点击取消");
					}
