	...
	<view class="flow-wrapper">
		...
		<view class="flow-list">
			...
			<template v-if="order_type == 1">
				<view class="flow-item">
					<uv-icon
						:name="dynamicIcon(out_warehouse_status)"
						:color="dynamicIconColor(out_warehouse_status)"
						label="调出仓确认"
						size="20"
						label-color="#000"
						space="10"
					></uv-icon>
					<view class="flow-item-info" v-for="(item, index) in outWarehouseList" :key="item.id">
						<text class="item-wh">{{ item.warehouse_name }}：</text>
						<text class="item-name">{{ item.name }}</text>
						<text class="item-dept">{{ item.dept_name }}</text>
					</view>
					<text class="line" :class="{ success: out_warehouse_status == 1, warning: out_warehouse_status == 2 }"></text>
				</view>
				<view class="flow-item">
					<uv-icon
						:name="dynamicIcon(to_warehouse_status)"
						:color="dynamicIconColor(to_warehouse_status)"
						label="调入仓确认"
						size="20"
						label-color="#000"
						space="10"
					></uv-icon>
					<view class="flow-item-info" v-for="(item, index) in toWarehouseList" :key="item.id">
						<text class="item-wh">{{ item.warehouse_name }}：</text>
						<text class="item-name">{{ item.name }}</text>
						<text class="item-dept">{{ item.dept_name }}</text>
					</view>
					<text class="line" :class="{ success: to_warehouse_status == 1, warning: to_warehouse_status == 2 }"></text>
				</view>
			</template>
 ...
 * @property {Number} order_type: 单据类型；1：调拨单;2：采购入库单;3：其它入库单;4：退库清单;5：退料入库单
...
export default {
	...
	data() {
		return {
			...
			warehouse_status: 0,
			// 调出仓库确认状态（仅调拨单有）0：未处理（灰色）、1：已确认（蓝色）2：进行中（橙色）
 			out_warehouse_status: 0,
	...
	methods: {
		async getData() {
			...
			this.out_warehouse_status = result.data.out_warehouse_status;
			this.to_warehouse_status = result.data.to_warehouse_status;
	...
	computed: {
		...
		outWarehouseList() {
		  return this.warehouseList.filter((item) => {
		    return item.warehouse_id.includes(this.outWhId);
		  });
		},
		// 过滤后的调入仓库确认人
		toWarehouseList() {
		  return this.warehouseList.filter((item) => {
		    return item.warehouse_id.includes(this.toWhId);
		  });
		},
		dynamicIconColor() {
			return (status) => {
