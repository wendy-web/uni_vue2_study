			...
			<view class="main">
				<wtabs-swiper>
					<template v-slot:detail>
						<view class="content">
							<wdetail-info :detailInfo="info" :type="9"></wdetail-info>
							<wgoods-list :type="9" :goods="info.goods"></wgoods-list>
						</view>
						<flow-vue
							:order_type="1" :order_id="order_id"
							:status="info.status"
							:toWhId="info.to_wh_id"
							:outWhId="info.out_wh_id"
						></flow-vue>
					</template>
					<template v-slot:log>
						<!-- 单据日志 -->
...
export default {
	...
	methods: {
		async getData() {
			...
			const result = await detailAllotApi({ id: this.order_id });
