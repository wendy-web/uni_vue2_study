				...
				<uv-swipe-action>
					<template v-for="(item, index) in dataList">
						...
						<uv-swipe-action-item
							...
							:disabled="!item.assoc_type.includes(1) || item.status != 6 || !checkDelBtn"
						>
							<wlist-item
								:type="9"
								@tapDetail="tapDetail"
								...
								@tapReject="tapReject"
								@tapWhInOutApprove="tapWhInOutApprove"
...
export default {
	...
	methods: {
		async upCallback(page) {
			...
			try {
				const result = await getAllotListApi(data);
				let res = result.data;
		...
		async tapStatus(e) {
			...
			const result = await getapproveLogApi({ document_type: 5, document_id: e.id });
		...
		async tapSubmit(e) {
			// uni.$uv.toast("点击提审");
			let id = e.id;
			const result = await submitAllotApi({ id });
		...
		async tapVoid(e, item) {
			...
			uni.showModal({
				title: "温馨提示",
				content: `您确定要作废【${wh_tra_no}】调拨单吗?`,
				success: async (res) => {
					if (res.confirm) {
						console.log("用户点击确定");
						let result = await voidAllotApi({ id });
						this.toastRefresh(result.msg);
					} else if (res.cancel) {
						console.log("用户点击取消");
					}
		...
		async tapRecall(e) {
			let id = e.id;
			const result = await recallAllotApi({ id });
			this.toastRefresh(result.msg);
		},
		// 触发点击通过
		async tapApprove(e) {
			let id = e.id;
			const result = await approveAllotApi({ id });
			this.toastRefresh(result.msg);
		},
		// 触发点击驳回
		tapReject(e) {
		...
		// 触发时间选择
		tapWhInOutApprove(event) {
			let { id, in_time, out_time, status } = event;
			
			this.$refs.submitDiaRef.open({ id, in_time, out_time, status });
		},
		// 入库弹窗确认
		...
		async rejectConfirm() {
			...
			// 审核驳回
			const result = await rejectAllotApi(data);
		...
		handleAction(name, item) {
			let { id, wh_tra_no } = item;
			uni.showModal({
				title: "温馨提示",
				content: `您确定要删除【${wh_tra_no}】调拨单吗?`,
				success: async (res) => {
					if (res.confirm) {
						console.log("用户点击确定");
						let result = await delAllotApi({ id });
						this.toastRefresh(result.msg);
					} else if (res.cancel) {
						console.log("用户点击取消");
					}
