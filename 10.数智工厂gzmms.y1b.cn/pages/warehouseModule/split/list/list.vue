...
export default {
	...
	methods: {
		...
		async tapStatus(e) {
			...
			const result = await getapproveLogApi({ document_type: 11, document_id: e.id });
		...
		async tapVoid(e, item) {
			// uni.$uv.toast("点击作废");
			let id = e.id;
			let { split_assemble_no } = item;
			uni.showModal({
				title: "温馨提示",
				content: `您确定要作废【${split_assemble_no}】拆装单吗?`,
				success: async (res) => {
					if (res.confirm) {
						console.log("用户点击确定");
						let result = await voidSplitApi({ id });
						this.toastRefresh(result.msg);
					} else if (res.cancel) {
						console.log("用户点击取消");
					}
		...
		async tapRecall(e) {
			let id = e.id;
			const result = await recallSplitApi({ id });
			this.toastRefresh(result.msg);
		},
		// 触发点击通过
		async tapApprove(e) {
			let id = e.id;
			const result = await approveSplitApi({ id });
			this.toastRefresh(result.msg);
		},
		// 触发点击驳回
		tapReject(e) {
		...
		handleAction(name, item) {
			...
			let { id, split_assemble_no } = item;
			uni.showModal({
				title: "温馨提示",
				content: `您确定要删除【${split_assemble_no}】拆装单吗?`,
				success: async (res) => {
					if (res.confirm) {
						console.log("用户点击确定");
						let result = await delSplitApi({ id });
						this.toastRefresh(result.msg);
					} else if (res.cancel) {
						console.log("用户点击取消");
					}
