...
<script>
import {
	getMaintenancePlanApi,
	scanOrderDetailsApi,
	OrderCreateApi,
	OrderDetailsApi,
	OrderEditApi
} from "@/api/device/maintain/order.js";
...
import { scanScrollList, allScrollList } from "./index.js";
export default {
	components: {
		changeUnit,
		planInfo,
	...
	methods: {
		...
		async addInitData() {
			...
			if(this.planData) {
				this.$refs.checkInfoRef && this.$refs.checkInfoRef.upDateForm(this.planData);
		...
		async initData() {
			this.detailLoading = true;
			const requestApi = this.isLoginShow ? OrderDetailsApi : scanOrderDetailsApi;
