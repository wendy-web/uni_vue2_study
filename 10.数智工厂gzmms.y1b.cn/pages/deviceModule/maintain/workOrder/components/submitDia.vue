...
export default {
	...
	methods: {
		...
		showTimeSelect(type) {
			...
			this.minDateValue = (type == 2) ? Date.parse(this.formData.maintenance_start_time) : '';
		...
		async open(item) {
			const { id, maintenance_start_time, complete_time } = item;
			this.formData = {
				id,
				maintenance_start_time,
				complete_time: complete_time || dayJs().format("YYYY-MM-DD HH:mm")
			}
