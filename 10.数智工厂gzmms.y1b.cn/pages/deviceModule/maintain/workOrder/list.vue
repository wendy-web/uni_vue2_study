  ...
  <view class="container">
    <uv-sticky offsetTop="0">
      ...
      <wdrop-type
        ...
        :noValidationLogin="noValidationLogin"
      ></wdrop-type>
    </uv-sticky>
...
<script>
import { commitAcceptOrderApi, getOrderListApi, getScanOrderListApi } from "@/api/device/maintain/order.js";
  ...
  export default {
    ...
    data() {
      return {
        ...
        noValidationLogin: false, // 是否校验登录态
      };
    },
    computed: {
		  ...mapGetters(['token']),
    ...
    onLoad(options) {
      ...
      if(options.noValidationLogin) this.noValidationLogin = Boolean(options.noValidationLogin);
    },
    methods: {
      checkAssocType,
      verifyRole(key) {
      ...
      toDetailHandle(id = 0, operateType = 1) {
        if(!this.isLoginShow) {
          uni.navigateTo({
            url: `./detail?id=${id}&operateType=3&noValidationLogin=${this.noValidationLogin}`,
          });
          return;
        }
      ...
      async upCallback(page) {
        ...
        const requestApi = this.isLoginShow ? getOrderListApi : getScanOrderListApi;
        const res = await requestApi(params).catch(() => this.mescroll.endErr());
