			...
			>
				...
				>
					<view class="width-full display_row_between_center f-s-28 t-w-bold">
						<view v-if="apiType == 'down'">换上时间<text class="f-s-24 t-c-aaa all-m-l-10">{{ formartDate(item.chage_date) }}</text></view>
						<block v-else>
					...
					<view class="width-full display_row_between_center f-s-28 t-w-bold all-m-t-10">
						...
						<view v-if="apiType == 'down'">在线数 <text class="all-m-l-10" style="color: #02A7F0;">{{ item.online_num }}</text></view>
						<view v-else>待用数 <text class="all-m-l-10" style="color: #02A7F0;">{{ item.no_use_num }}</text></view>
					</view>
...
export default {
	...
	onLoad(option) {
		if(option.apiType) {
			...
			(option.apiType == 'down') && uni.setNavigationBarTitle({ title: "选择换下备件" });
