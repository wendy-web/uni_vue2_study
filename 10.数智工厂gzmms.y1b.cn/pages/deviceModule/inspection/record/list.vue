...
<script>
import { getInspecRecordListApi, getScanRecordListApi } from "@/api/device/inspection/record.js";
...
export default {
	...
	methods: {
		...
		checkRectifyText(is_report_rectify, rectify_status_text) {
			if (is_report_rectify === 1)  return rectify_status_text;
			return "无需整改";
		},
		/** 检查提交验收按钮是否禁用 */
		...
		cellDetail(data) {
			let { id } = data;
			uni.navigateTo({
				url: `./detail?id=${id}&noValidationLogin=${this.noValidationLogin}`,
			});
		},
		cellSubmit(data) {
			let { id } = data;
		...
		async upCallback(page) {
			...
			try {
				const requestApi = this.isLoginShow ? getInspecRecordListApi : getScanRecordListApi;
