<template>
	<view class="width-full scrollable-content">
		...
		<view class="width-full all-p-t-30 all-p-lr-20">
			<topInfoVue :info="planData" id="one" :isLoginShow="isLoginShow">
...
import { changeDetail, scanScrollList, allScrollList } from "./index.js";
...
export default {
	...
	data() {
		return {
			...
			detailLoading: true,
			noValidationLogin: false
		};
	},
	// 生命周期 - 监听页面加载
	onLoad(options) {
		this.listId = options.id ? Number(options.id) : 0;
		this.noValidationLogin = options.noValidationLogin ? Boolean(options.noValidationLogin) : false;
		if (this.listId) {
			this.getEditData();
		}
	},
	...
	computed: {
		...mapGetters(['token']),
		isLoginShow() {
			return Boolean(!this.noValidationLogin || this.token);
		},
		scrollList() {
			return this.isLoginShow ? allScrollList : scanScrollList;
		}
	...
	methods: {
		...
		getElementPositions() {
			...
			const elements = this.scrollList.map(res => res.scrollId);
		...
		async getEditData() {
			...
			const requestApi = this.isLoginShow ? getInspecRecordDetailApi : getScanInspecRecordDetailApi;
			const result = await requestApi({ id: this.listId });
			...
			if(this.$refs.checkInfoRef) {
				...
				// 	: []; //执行人
				data.executor_user_text && (this.$refs.checkInfoRef.formData.executor_name = data.executor_user_text);
			}
