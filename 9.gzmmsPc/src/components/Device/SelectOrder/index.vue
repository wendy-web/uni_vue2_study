...
import { getWarehouseRecDetailApi } from "@/api/device/common/index";
import type { ReBaseSelecModule } from "@/api/device/common/types";
import { useSelectOrder } from "./hook";
...
const { columns, searchColumns, pagination } = useSelectOrder();
...
const formRef = ref();
const tableData = ref<ReBaseSelecModule.OrderUpListType[]>([]);
const selectData = ref<ReBaseSelecModule.OrderUpListType[]>([]); //勾选的数据列表
...
function changeSelect(selection: ReBaseSelecModule.OrderUpListType[]) {
  selectData.value = selection;
}
// 按钮选择单元格事件
const toggleSelection = (row: ReBaseSelecModule.OrderUpListType) => {
...
function checkId() {
  tableData.value.forEach((item) => {
    if (idsList.value.includes(item.rec_detail_id)) {
...
const setStatus = () => {
  tableData.value.forEach((item, index) => {
    if (selectList.value.includes(item.rec_detail_id)) {
      item.select_status = true;
    }
  });
  selectTableRef.value!.clearSelection();
...
async function getData() {
  ...
  const result = await getWarehouseRecDetailApi(data);
