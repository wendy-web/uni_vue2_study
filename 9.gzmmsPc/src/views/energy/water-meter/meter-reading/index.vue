...
import {
  getWaterMeterReadListApi,
  waterMeterReadAddApi,
  waterMeterReadDelApi,
  waterMeterReadDetailApi,
  waterMeterReadEditApi,
} from "@/api/energy/water-meter/meter-reading/index";
import { WaterRecordListType } from "@/api/energy/water-meter/meter-reading/types";
...
const tableData = ref<WaterRecordListType[]>([]);
...
async function getData() {
  ...
  const result = await getWaterMeterReadListApi(data);
...
function showMeterConfig() {
  addDialog({
    ...
    beforeSure: async (done) => {
      ...
      let { equipment_type_id, use_addr, use_addr_text, eq_id } =
      ...
      try {
        const result = listId.value
          ? await waterMeterReadEditApi({ ...data, id: listId.value })
          : await waterMeterReadAddApi(data);
...
function cellEdit(row: WaterRecordListType) {
...
function cellDel(row: WaterRecordListType) {
  ElMessageBox.confirm(`确认要删除抄表流水号为：【${row.serial_number_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await waterMeterReadDelApi({ id: row.id });
      ElMessage.success(result.msg);
      getData();
    })
    .catch((error) => {
        ...
        <PureTableBar :columns="columns" @refresh="handleSearch">
          <template #buttons>
            <el-button
              type="primary"
              @click="handleAdd"
              v-hasPerm="['watermeter:meterreading:add']"
            >
              <template #icon>
                <i-ep-plus></i-ep-plus>
              </template>
            ...
            >
              <template #operation="{ row }">
                <el-button
                  ...
                  link
                  @click="cellEdit(row)"
                  v-hasPerm="['watermeter:meterreading:edit']"
                >
                  编辑
                </el-button>
                <el-button
                  ...
                  link
                  @click="cellDel(row)"
                  v-hasPerm="['watermeter:meterreading:del']"
                >
                  删除
                </el-button>
              </template>
