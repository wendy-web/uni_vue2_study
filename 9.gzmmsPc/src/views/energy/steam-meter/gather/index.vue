...
import {
  getSteamMeterListApi,
  steamMeterAddApi,
  steamMeterEditApi,
} from "@/api/energy/steam-meter/gather/index";
...
async function getData() {
  ...
  const result = await getSteamMeterListApi(data);
    ...
    beforeSure: async (done) => {
      ...
      try {
        const result = listId.value
          ? await steamMeterEditApi({ ...data, id: listId.value })
          : await steamMeterAddApi(data);
  ...
  <div class="app-container">
    ...
    <div class="app-card">
      <PureTableBar :columns="columns" @refresh="handleSearch">
        <template #buttons>
          <el-button type="primary" @click="handleAdd" v-hasPerm="['steammeter:gather:add']">
            <template #icon>
              <i-ep-plus></i-ep-plus>
            </template>
            新增仪表配置
          ...
          >
            <template #operation="{ row }">
              <el-button
                ...
                v-hasPerm="['steammeter:gather:info']"
              >
                读数明细
              </el-button>
              <el-button
                ...
                link
                @click="cellEdit(row)"
                v-hasPerm="['steammeter:gather:edit']"
              >
                编辑配置
              </el-button>
            </template>
