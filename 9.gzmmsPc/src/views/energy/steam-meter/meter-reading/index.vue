...
import {
  getSteamMeterReadListApi,
  steamMeterReadAddApi,
  steamMeterReadDelApi,
  steamMeterReadEditApi,
} from "@/api/energy/steam-meter/meter-reading";
...
async function getData() {
  ...
  const result = await getSteamMeterReadListApi(data);
    ...
    beforeSure: async (done) => {
      ...
      try {
        const result = listId.value
          ? await steamMeterReadEditApi({ ...data, id: listId.value })
          : await steamMeterReadAddApi(data);
...
function cellDel(row: any) {
  ElMessageBox.confirm(`确认要删除抄表流水号为：【${row.serial_number_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await steamMeterReadDelApi({ id: row.id });
      ElMessage.success(result.msg);
      getData();
    })
    .catch((error) => {
        ...
        <PureTableBar :columns="columns" @refresh="handleSearch">
          <template #buttons>
            <el-button
              type="primary"
              @click="handleAdd"
              v-hasPerm="['steammeter:meterreading:add']"
            >
              <template #icon>
                <i-ep-plus></i-ep-plus>
              </template>
            ...
            >
              <template #operation="{ row }">
                <el-button
                  ...
                  link
                  @click="cellEdit(row)"
                  v-hasPerm="['steammeter:meterreading:edit']"
                >
                  编辑
                </el-button>
                <el-button
                  ...
                  link
                  @click="cellDel(row)"
                  v-hasPerm="['steammeter:meterreading:del']"
                >
                  删除
                </el-button>
              </template>
