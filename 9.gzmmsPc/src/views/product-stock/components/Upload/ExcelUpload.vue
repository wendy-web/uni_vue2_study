<template>
  <el-dialog title="导入" v-model="visibleDialog" width="30%" draggable>
...
import { importApi } from "@/api/product-stock/product-wscode/index";
...
downloadMap.set(1, baseUrl + "/static/库位导入模版.xlsx");
...
// 货品导入时用的方法和api
async function importExcelFn(options: UploadRequestOptions) {
  ...
  try {
    const result = await importApi(options.file);
