...
function handleCancel() {
  router.replace({
    path: "/quality/environment/cleanroom-bacteria",
  });
}
// 点击保存：1-保存，2-提交
async function handleSave(type = 1) {
  ...
  initCheckInfoRef.tableFormRef.validate(async (valid: boolean, fields: any) => {
    if (valid) {
      // 检验信息
      formData.value.check_info = [checkTableData.value];
      // 附件信息
...
async function getDetailData() {
  try {
    ...
    formData.value.culture_end_time = res.culture_end_time;
    formData.value.culture_temperature = res.culture_temperature;
    formData.value.pressure_diff_val = res.pressure_diff_val;
    ...
    if(res.check_info.length>0){
      checkTableData.value = res.check_info[0];
    }else{
      initCheckTableData();
    }
    // nextTick(async () => {
...
<template>
  <div class="app-container !pb-[40px]">
    <AffixButton v-bind="affixBtnData" :order-type="40" v-on="affixBtnEvent"></AffixButton>
        ...
        >
          ...
          <el-collapse-item name="2" class="mt-2">
            ...
            <div class="ml-8 mt-4 pl-2 flex align-center border-l-4 border-sky-500">
              <div>备注:</div>
              <div class="ml-2">每个点采样量为100L/次。</div>
            </div>
          </el-collapse-item>
