  ...
  <div class="app-box !p-0 flex-1">
    <div class="pure-table-box">
      ...
      >
        ...
        >
          <template #germ_val="{ row, $index }">
            <el-form-item
              :prop="`checkTableData.${$index}.germ_val`"
              :rules="checkFormRules.germ_val"
            >
              <el-input
                v-model="row.germ_val"
                placeholder="请输入内容"
                @input="handleInputChange($event, $index)"
              ></el-input>
            </el-form-item>
          </template>
          <template #mould_val="{ row, $index }">
            <el-form-item
              :prop="`checkTableData.${$index}.mould_val`"
              :rules="checkFormRules.mould_val"
            >
              <el-input
                v-model="row.mould_val"
                placeholder="请输入内容"
                @input="handleInputChange($event, $index)"
              ></el-input>
            </el-form-item>
          </template>
          <template #yeast_val="{ row, $index }">
            <el-form-item
              :prop="`checkTableData.${$index}.yeast_val`"
              :rules="checkFormRules.zipper_val"
            >
              <el-input
                v-model="row.yeast_val"
                placeholder="请输入内容"
                @input="handleInputChange($event, $index)"
              ></el-input>
            </el-form-item>
