...
import {
  ...
  reviewrderApi,
  revokeOrderApi,
} from "@/api/quality/environment/fingerprint-film/index";
// 签名组件
...
function handleCancel() {
  router.replace({
    path: "/quality/environment/fingerprint-film",
  });
}
// 点击保存：1-保存，2-提交
async function handleSave(type = 1) {
  ...
  initCheckInfoRef.tableFormRef.validate(async (valid: boolean, fields: any) => {
    if (valid) {
      // 检验信息
      const { germ_val = "", mould_val = "", yeast_val = "" } = checkTableData.value[0];
      formData.value.germ_val = germ_val;
      formData.value.mould_val = mould_val;
      formData.value.yeast_val = yeast_val;
      // 附件信息
...
async function getDetailData() {
  try {
    ...
    formData.value.tested_uid = res.tested_uid;
    formData.value.check_uid = res.check_uid;
    formData.value.check_user_signature = res.check_user_signature;
    formData.value.germ_val = res.germ_val;
    formData.value.mould_val = res.mould_val;
    formData.value.yeast_val = res.yeast_val;
...
<template>
  <div class="app-container !pb-[40px]">
    <AffixButton v-bind="affixBtnData" :order-type="39" v-on="affixBtnEvent"></AffixButton>
