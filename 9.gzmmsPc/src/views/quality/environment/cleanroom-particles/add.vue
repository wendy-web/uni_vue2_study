...
import {
  ...
  reviewrderApi,
  revokeOrderApi,
} from "@/api/quality/environment/cleanroom-particles/index";
import {
...
function handleCancel() {
  router.replace({
    path: "/quality/environment/cleanroom-particles",
  });
}
// 点击保存：1-保存，2-提交
...
async function getDetailData() {
  try {
    ...
    formData.value.area_id = res.area_id;
    formData.value.check_date = res.check_date;
    formData.value.level = res.level;
    formData.value.check_type = res.check_type;
    formData.value.temperature = res.temperature;
    formData.value.humidity = res.humidity;
    formData.value.ucl5_val = res.ucl5_val;
    formData.value.ucl05_val = res.ucl05_val;
    ...
    // 获取标准浓度基础信息
    if(res.level){
     let standardInfoRes = await getParticleStandardInfo({level:res.level});
     standardInfo.value = standardInfoRes?.data.list;
    }
    // nextTick(async () => {
    //   let initCheckInfoRef = checkInfoRef.value as any;
...
<template>
  <div class="app-container !pb-[40px]">
    <AffixButton v-bind="affixBtnData" :order-type="37" v-on="affixBtnEvent"></AffixButton>
