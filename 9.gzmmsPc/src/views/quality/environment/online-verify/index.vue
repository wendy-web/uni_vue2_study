...
function handleExport() {
  ...
  startDownloadUrl(onlineVerifyExportApi, { ids: ids.value });
}
...
async function getData() {
  ...
  const result = await onlineVerifyListApi(data);
...
const handleAddConfirm = async (handleSubmit: () => Promise<boolean>, type = 1) => {
  ...
  const result = await onlineVerifyAddApi(data);
  ...
  if (type === 1) {
    ...
    router.push({
      path: "/quality/environment/online-verify/add",
      query: {
        id,
        pageType: 2,
      },
...
function cellDel(row: any) {
  ElMessageBox.confirm(`确认要删除单据编号为：【${row.order_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await onlineVerifyDelApi({ id: row.id });
...
async function cellGenerateReport(row: any) {
  startDownloadUrl(onlineVerifyExportApi, { id: row.id });
}
      ...
      <PureTableBar :columns="columns" @refresh="handleSearch">
        <template #buttons="scope">
          <el-button
            ...
            @click="handleAdd"
            :icon="Plus"
            v-hasPerm="['environment:ciphygiene:add']"
          >
            新建
          </el-button>
          <el-button
            type="primary"
            @click="handleExport"
            v-hasPerm="['environment:ciphygiene:report']"
          >
            导出选中数据
          </el-button>
        </template>
      ...
      <template #form-footer="{ handleSubmit }">
        <div class="flex justify-center mt-10 w-full">
          ...
          <el-button
            ...
            v-hasPerm="['environment:onlineverify:execute']"
            size="large"
          >
            确认并执行
          </el-button>
