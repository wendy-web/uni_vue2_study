...
export function useList(search: () => void) {
  ...
  const { getLineList, getUserList, getClasstimeMap } = useselectData();
  ...
  const formData = ref({
    order_no: "", // 单据编号
    class_id: undefined as FormNumType, //班次id
  ...
  const selectClassRef = ref<InstanceType<typeof ElSelect>>();
  ...
  const addFormColumns: PlusColumnList = [
    ...
    },
    {
      ...
      fieldProps: {
        ref: (instance: InstanceType<typeof ElSelect>) => {
          selectClassRef.value = instance;
        },
        onChange(value: number) {
          nextTick(() => {
            addFormData.value.class_name = selectClassRef.value?.selected.currentLabel;
          });
        },
      },
      options: getClasstimeMap,
