...
function handleCancel() {
  router.replace({
    path: "/quality/environment/fly-lamp",
  });
}
...
async function handleSubmit() {
  ...
  addDialog({
    ...
    beforeSure: async (done) => {
      ...
      try {
        const result = await flyLampConfirmApi({ id: listId.value, sign: signRes });
...
function handleReverse() {
  ElMessageBox.confirm(`确认要反审核该单据吗?`, "警告", {
    ...
    .then(async () => {
      const result = await flyLampReverseApi({ id: listId.value });
      ElMessage.success(result.msg);
      closeCurrentPage(result.msg);
    })
    .catch((error) => {
...
function handleDel() {
  ...
  ElMessageBox.confirm(`确认要删除单据编号为：【${order_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await flyLampDelApi({ id: listId.value });
      ElMessage.success(result.msg);
      closeCurrentPage(result.msg);
    })
    .catch((error) => {
...
async function getDetailData() {
  ...
  const result = await flyLampDetailApi({
    id: listId.value,
  });
  ...
  baseForm.value.dept_name = res.dept_name;
