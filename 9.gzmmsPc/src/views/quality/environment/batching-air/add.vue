...
import {
  ...
  reviewrderApi,
  revokeOrderApi,
} from "@/api/quality/environment/batching-air/index";
// 签名组件
...
function handleCancel() {
  router.replace({
    path: "/quality/environment/batching-air",
  });
}
// 点击保存：1-保存，2-提交
async function handleSave(type = 1) {
  ...
  initCheckInfoRef.tableFormRef.validate(async (valid: boolean, fields: any) => {
    if (valid) {
      ...
      // console.log("传参data:", data.check_info);
...
async function getDetailData() {
  try {
    ...
    // console.log("初始化数据: ", checkTableData.value);
    // nextTick(async () => {
    //   let initCheckInfoRef = checkInfoRef.value as any;
...
<template>
  <div class="app-container !pb-[40px]">
    <AffixButton v-bind="affixBtnData" :order-type="35" v-on="affixBtnEvent"></AffixButton>
          ...
          <el-collapse-item name="4" class="mt-2">
            ...
            <div class="pl-8">
              ...
              <el-space alignment="flex-start" direction="vertical">
                ...
                <el-text>
                  ③培养结束后，由品管经理或化验主管对结果进行计数，方法为用肉眼对培养皿上所有的菌落直接计数，统计菌落数。化验员将检测结果记录于《称配料间空气沉降检测报告》中，并计算每条线沉降菌菌落数平均值。
                </el-text>
              </el-space>
            </div>
            <div class="pl-8">
              ...
              <el-space alignment="flex-start" direction="vertical">
                ...
                <el-text>
                  ②当称配料间区域某个测试点的沉降菌菌落数超过判定标准时，应及时通知生产部进行卫生清洁后，采取双倍量采样，每个培养皿检测结果合格才能判定合格。
                </el-text>
              </el-space>
            </div>
            <div class="pl-8">
            ...
            <div class="pl-8">
              ...
              <el-space alignment="flex-start" direction="vertical">
                <el-text>1.采样点：配料间14个点（共7个房间，每个房间2个点）；</el-text>
                <el-text>2.称配料间温度15℃-28℃、相对湿度45%-85%</el-text>
