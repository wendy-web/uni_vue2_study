...
import {
  cipHygieneConfirmApi,
  cipHygieneDelApi,
  cipHygieneDetailApi,
  cipHygieneReverseApi,
...
function handleCancel() {
  router.replace({
    path: "/quality/environment/cip-hygiene",
  });
}
...
async function handleSubmit() {
  ...
  addDialog({
    ...
    beforeSure: async (done) => {
      ...
      try {
        const result = await cipHygieneConfirmApi({ id: listId.value, sign: signRes });
...
function handleReverse() {
  ElMessageBox.confirm(`确认要反审核该单据吗?`, "警告", {
    ...
    .then(async () => {
      const result = await cipHygieneReverseApi({ id: listId.value });
      ElMessage.success(result.msg);
      closeCurrentPage(result.msg);
    })
    .catch((error) => {
...
function handleDel() {
  ...
  ElMessageBox.confirm(`确认要删除单据编号为：【${order_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await cipHygieneDelApi({ id: listId.value });
      ElMessage.success(result.msg);
      closeCurrentPage(result.msg);
    })
    .catch((error) => {
...
async function getDetailData() {
  ...
  const result = await cipHygieneDetailApi({
    id: listId.value,
  });
