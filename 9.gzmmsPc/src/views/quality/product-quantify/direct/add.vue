...
function handleCancel() {
  router.replace({
    path: "/quality/product-quantify/direct",
  });
}
// 点击保存：1-保存，2-提交
...
async function getDetailData() {
  try {
    ...
    formData.value.sample_batch_no = res.sample_batch_no;
    formData.value.line_id = res.line_id;
    formData.value.line_name = res.line_name;
    formData.value.make_date = res.make_date;
    formData.value.insp_id = res.insp_id;
    formData.value.insp_name = res.insp_name;
...
onActivated(() => {
  ...
  // 初始化线别下拉选项
  initProductLineList();
  let directAddParams = storageSession().getItem<any>("directAddParams");
  if (pageType.value === 1) {
    if (!directAddParams) {
    ...
    batchParams.value = directAddParams;
    let { brand, sku, batch_no, sample_batch_no, make_date } = directAddParams;
    formData.value.brand = brand;
    formData.value.sku = sku;
    formData.value.batch_no = batch_no;
    formData.value.sample_batch_no = sample_batch_no;
    formData.value.make_date = make_date;
...
<template>
  <div class="app-container !pb-[40px]">
    <AffixButton v-bind="affixBtnData" :order-type="22" v-on="affixBtnEvent"></AffixButton>
