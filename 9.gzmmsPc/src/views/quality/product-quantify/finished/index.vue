...
const handleRowClick = (row: any, column: Column) => {
  // 点击报告编号 打开详情不可编辑
  if (column.property === "report_no") {
    router.push({
      path: "/quality/product-quantify/finished/add",
      query: {
        pageType: 3,
        id: row.id,
...
async function addConfirm(values: FieldValues) {
  ...
  const result = await submitOrderApi({ batch_no: values.batch_no });
  router.push({
    path: "/quality/product-quantify/finished/add",
    query: {
      pageType: 1,
