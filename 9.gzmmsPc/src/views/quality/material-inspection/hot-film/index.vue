...
const tableData = ref<HotFilmModule.ListItem[]>([]);
...
const { formData, searchColumns, columns, pagination, getBaseData, BaseData } =
...
const handleChange = (values: FieldValues, column: PlusColumn) => {
  ...
  if (["brand"].includes(prop) && values[prop]) {
    ...
    if (matchingBrand) {
      ...
      formData.value.film = brandChild[0].id;
    }
    // console.log("formData.value.sku:", formData.value.sku);
  }
};
// 点击重置
...
// 表格行-点击事件
const handleRowClick = (row: HotFilmModule.ListItem, column: Column) => {
  // 点击单据编号 打开详情不可编辑
  if (column.property === "order_no") {
    router.push({
      path: "/quality/material-inspection/hot-film/add",
      query: {
        pageType: 3,
        id: row.id,
...
// 单元格点击撤回
const cellRecall = (row: HotFilmModule.ListItem) => {
...
// 单元格点击删除
const cellDel = (row: HotFilmModule.ListItem) => {
