...
import type { LabelListType } from "@/api/quality/material-inspection/label/types";
...
const tableData = ref<LabelListType[]>([]);
...
async function getData() {
  ...
  const result = await getLabelListApi(data);
...
// 勾选触发事件
function changeSelect(selection: LabelListType[]) {
...
function handleAdd() {
  router.push({
    path: "/quality/material-inspection/empty-cans/label/add",
  });
}
...
/** 点击编辑 */
function cellEdit(row: LabelListType) {
  router.push({
    path: "/quality/material-inspection/empty-cans/label/add",
    query: {
      id: row.id,
...
/** 点击去提交 */
function cellSubmit(row: LabelListType) {
  router.push({
    path: "/quality/material-inspection/empty-cans/label/add",
    query: {
      id: row.id,
...
/** 点击删除 */
function cellDel(row: LabelListType) {
  ElMessageBox.confirm(`确认要删除单据编号为：【${row.order_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await labelDelApi({ id: row.id });
...
/** 点击撤回 */
async function cellRecall(row: LabelListType) {
  const result = await labelRecallApi({ id: row.id });
  ElMessage.success(result.msg);
  getData();
}

/** 点击生成报告 */
function cellGenerateReport(row: LabelListType) {
  startDownloadUrl(labelReportApi, { id: row.id });
}
    ...
    <div class="app-card">
      <PureTableBar :columns="columns" @refresh="handleSearch">
        <template #buttons="scope">
          <el-button type="primary" @click="handleAdd" :icon="Plus" v-hasPerm="['mi:label:add']">
            新建
          </el-button>
