...
import type { CansStockListType } from "@/api/quality/material-inspection/cans-stock/types";
...
const tableData = ref<CansStockListType[]>([]);
...
async function getData() {
  ...
  const result = await getCansStockListApi(data);
...
// 勾选触发事件
function changeSelect(selection: CansStockListType[]) {
...
function handleAdd() {
  router.push({
    path: "/quality/material-inspection/empty-cans/cans-stock/add",
  });
}
...
/** 点击编辑 */
function cellEdit(row: CansStockListType) {
  router.push({
    path: "/quality/material-inspection/empty-cans/cans-stock/add",
    query: {
      id: row.id,
...
/** 点击去提交 */
function cellSubmit(row: CansStockListType) {
  router.push({
    path: "/quality/material-inspection/empty-cans/cans-stock/add",
    query: {
      id: row.id,
...
/** 点击删除 */
function cellDel(row: CansStockListType) {
  ElMessageBox.confirm(`确认要删除单据编号为：【${row.order_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await cansStockDelApi({ id: row.id });
...
/** 点击撤回 */
async function cellRecall(row: CansStockListType) {
  const result = await cansStockRecallApi({ id: row.id });
  ElMessage.success(result.msg);
  getData();
}

/** 点击生成报告 */
function cellGenerateReport(row: CansStockListType) {
  startDownloadUrl(cansStockReportApi, { id: row.id });
}
