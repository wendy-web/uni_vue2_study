...
const tableData = ref<CansSeamModule.CansSeamItem[]>([]);
...
const { formData, searchColumns, columns, pagination, getEmptyOrderBaseData } =
...
// 表格行-点击事件
const handleRowClick = (row: CansSeamModule.CansSeamItem, column: Column) => {
  // 点击单据编号 打开详情不可编辑
  if (column.property === "order_no") {
    router.push({
      path: "/quality/material-inspection/empty-cans/cans-seam/add",
      query: {
        pageType: 3,
        id: row.id,
...
// 单元格点击编辑
const cellEdit = (row: CansSeamModule.CansSeamItem) => {
  router.push({
    path: "/quality/material-inspection/empty-cans/cans-seam/add",
    query: {
      pageType: 2,
...
const cellDetail = (row: CansSeamModule.CansSeamItem) => {
  router.push({
    path: "/quality/material-inspection/empty-cans/cans-seam/add",
    query: {
      pageType: 3,
...
// 单元格点击撤回
const cellRecall = (row: CansSeamModule.CansSeamItem) => {
  ElMessageBox.confirm(`您确定要撤回单据【${row.order_no}】吗?`, "温馨提示", {
    ...
    .then(async () => {
      try {
        let result = await revokeEmptypotApi({ id: row.id });
...
// 单元格点击删除
const cellDel = (row: CansSeamModule.CansSeamItem) => {
  ElMessageBox.confirm(`确认要删除单据编号为：【${row.order_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      let result = await delEmptypotApi({ id: row.id });
      ElMessage.success(result.msg);
      getData();
    })
    .catch((error) => {
...
/** 点击生成报告 */
function cellGenerateReport(row: CansSeamModule.CansSeamItem) {
...
async function getData() {
  ...
  const result = await getEmptypotListApi(data);
