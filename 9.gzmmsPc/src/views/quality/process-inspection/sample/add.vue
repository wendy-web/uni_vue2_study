...
import {
  ...
  reviewrderApi,
  revokeOrderApi,
} from "@/api/quality/process-inspection/sample/index";
// 签名组件
...
async function beforeSubmit() {
  ...
  let hasSameBatch = await checkInfoRef.value!.checkSameBatch();
  // 批号重复弹窗提示
  if (hasSameBatch) {
    ElMessageBox.confirm("检验信息中存在重复批号, 确认提交?", "温馨提示", {
...
function getcheckInfo() {
  ...
  return checkTableData.value?.length > 0
    ? checkTableData.value.map((item) => {
        let { unique_id, Brix, overlap_rate, density, wrinkle, ...rest } = item;
        if (Brix) {
          Brix = Brix / 100;
        }
        if (overlap_rate) {
        ...
        return { Brix, overlap_rate, density, wrinkle, ...rest };
      })
    : undefined;
}
// 操作成功返回列表
...
async function getDetailData() {
  try {
    ...
    formData.value.sample_num = res.sample_num;
    formData.value.net_ret = res.net_ret;
    formData.value.check_date = res.check_date;
    formData.value.line_id = res.line_id;
    formData.value.line_id_name = res.line_id_name;
    formData.value.check_date = res.check_date;
    ...
    checkinfo.forEach((item: any) => {
      item.purity = Number(item.purity);
      if (item.Brix) {
        item.Brix = item.Brix * 100;
      }
      if (item.overlap_rate) {
        item.overlap_rate = item.overlap_rate * 100;
      }
      if (item.density) {
        item.density = item.density * 100;
      }
      if (item.wrinkle) {
        item.wrinkle = item.wrinkle * 100;
      }
    });
...
watch(
  ...
  (newValue) => {
    ...
    if (newValue?.length > 0) {
      ...
      // 计算净含量平均值,根据产品类型判断是否合格
      const average_net_ret =
        checkTableData.value.reduce((sum: any, item: any) => sum + Number(item.net), 0) /
        checkTableData.value.length;
      let { brand } = formData.value;
      if (brand === "ND1") {
        formData.value.net_ret = average_net_ret >= 250 ? 1 : 0;
      }
      if (brand === "ND2") {
        formData.value.net_ret = average_net_ret >= 310 ? 1 : 0;
      }
...
<template>
  <div class="app-container !pb-[40px]">
    <AffixButton v-bind="affixBtnData" :order-type="25" v-on="affixBtnEvent"></AffixButton>
