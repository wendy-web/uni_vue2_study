...
export function useList() {
  ...
  const formData = ref({
    ...
    order_no: "", // 单据编号
    batch_no: "", //批次
    status: undefined as FormNumType, //审核状态
    check_date: "", //检验日期
  });
  /** 签字复核/反审核,都是进入详情页面 */
  ...
  const columns: TableColumnList = [
    ...
    },
    {
      ...
      align: "center",
      cellRenderer: ({ row }) => {
        return typeof row.check_ret === "number" ? getBasePassLabel(row.check_ret) : "--";
      },
    },
    {
      label: "检验员",
      ...
      cellRenderer: ({ row }) => (
        <>
          <ElTag
            ...
            onClick={() => showApproveLog({ id: row.id, orderType: 26, orderStatus: row.status })}
          >
