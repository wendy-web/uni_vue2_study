...
wrapping.value.check_info = Array.from({ length: props.checkNum }, () => wrapping.value.check_info[0]);
...
<template>
  <el-form :disabled="isDetailDisable">
    <table>
      ...
      <tr>
        <td>时间</td>
        <td colspan="2" v-for="item in wrapping.check_info">
          <el-time-picker
        ...
        <td rowspan="8">
          <el-input v-model="wrapping.note" placeholder="备注" :rows="6" type="textarea"></el-input>
        </td>
      </tr>
      <tr>
        <td>塑封质量</td>
        <td colspan="2" v-for="item in wrapping.check_info">
          <el-input v-model="item.swq" placeholder="塑封质量"></el-input>
        </td>
      </tr>
      <tr>
        <td>环境卫生及岗位人员</td>
