...
import {
  getPowerConfirmListApi,
  powerConfirmAddApi,
  powerConfirmDelApi,
  powerConfirmReportApi,
...
async function getData() {
  ...
  const result = await getPowerConfirmListApi(data);
...
async function cellGenerateReport(row: any) {
  startDownloadUrl(powerConfirmReportApi, { id: row.id });
}

/** 点击编辑 */
function cellEdit(row: any) {
  router.push({
    path: "/quality/process-inspection/start/add",
    query: {
      id: row.id,
      pageType: 2,
    },
...
function cellDel(row: any) {
  ElMessageBox.confirm(`确认要删除单据编号为：【${row.order_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await powerConfirmDelApi({ id: row.id });
...
const handleAddConfirm = async (handleSubmit: () => Promise<boolean>, type = 1) => {
  ...
  const result = await powerConfirmAddApi(data);
  addLoading.value = false;
  addVisible.value = false;
    ...
    <div class="app-card">
      <PureTableBar :columns="columns" @refresh="handleSearch">
        <template #buttons="scope">
          <el-button type="primary" @click="handleAdd" :icon="Plus" v-hasPerm="['pi:start:add']">
            新建
          </el-button>
        </template>
