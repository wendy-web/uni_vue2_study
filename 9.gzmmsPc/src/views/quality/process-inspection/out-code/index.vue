...
const handleRowClick = (row: any, column: Column) => {
  ...
  if (column.property === "order_no") {
    router.push({
      path: "/quality/process-inspection/out-code/add",
      query: {
        pageType: 3,
        id: row.id,
...
async function getData() {
  let { make_date_arr, ...rest } = formData.value;

  let data = {
    page: pagination.currentPage,
    size: pagination.pageSize,
    make_date_start: isArray(make_date_arr) ? make_date_arr[0] : "",
    make_date_end: isArray(make_date_arr) ? make_date_arr[1] : "",
    ...rest,
  };
