...
import {
  ...
  reviewrderApi,
  revokeOrderApi,
} from "@/api/quality/process-inspection/out-code/index";
// 签名组件
...
import promptVue from "@/views/quality/components/RecheckSign/prompt.vue";
// 引入订单操作按钮
...
function handleCancel() {
  router.replace({
    path: "/quality/process-inspection/out-code",
  });
}
// 点击保存：1-保存，2-提交
...
// 复核审批组件
const recheckSignRef = ref<InstanceType<typeof promptVue>>();
...
watch(
  ...
  (newValue) => {
    ...
    if (newValue?.length > 0) {
      ...
      formData.value.abnormal = checkTableData.value.filter((item) => item.check_ret === 0).length;
