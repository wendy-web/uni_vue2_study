...
async function getData() {
  ...
  const result = await getCipMicrobesListApi(data);
...
function cellGenerateReport(row: any) {
  startDownloadUrl(cipMicrobesReportApi, { id: row.id });
}
...
function cellDel(row: any) {
  ElMessageBox.confirm(`确认要删除单据编号为：【${row.order_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await cipMicrobesDelApi({ id: row.id });
...
async function cellRecall(row: any) {
  const result = await cipMicrobesRecallApi({ id: row.id });
  ElMessage.success(result.msg);
  getData();
}
      ...
      <PureTableBar :columns="columns" @refresh="handleSearch">
        <template #buttons="scope">
          <el-button
            ...
            @click="handleAdd"
            :icon="Plus"
            v-hasPerm="['pi:cipmicrobes:addedit']"
          >
            新建
          </el-button>
        </template>
        <template v-slot="{ size, dynamicColumns }">
          ...
          >
            ...
            <template #check_sign2="{ row }">
              <el-image
                :src="useSetting.baseHttp + row.check_sign2"
                style="width: 100px; height: 60px; border-radius: 6px"
                :preview-src-list="[useSetting.baseHttp + row.check_sign2]"
                :z-index="9999"
                preview-teleported
                v-if="row.check_sign2"
              />
              <span v-else>--</span>
            </template>
