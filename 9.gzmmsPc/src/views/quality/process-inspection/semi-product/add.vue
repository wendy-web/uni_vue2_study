...
import {
  ...
  reviewrderApi,
  revokeOrderApi,
} from "@/api/quality/process-inspection/semi-product/index";
// 签名组件
...
function handleCancel() {
  router.replace({
    path: "/quality/process-inspection/semi-product",
  });
}
// 点击保存：1-保存，2-提交
...
watch(
  ...
  (newValue) => {
    ...
    if (newValue?.length > 0) {
      ...
      // 更新差值，如果CT和样品有值的情况，取绝对值Math.abs()
      checkTableData.value.forEach((item) => {
        if (item.CT) {
          let sample = item.sample || 0;
          item.delta = Math.abs(Number(item.CT) - Number(sample)).toFixed(2);
        }
      });
