...
import {
  getRinseRecordListApi,
  rinseRecordAddApi,
  rinseRecordConfirmApi,
  rinseRecordDelApi,
  rinseRecordEditApi,
  rinseRecordReportApi,
} from "@/api/quality/instrument/rinse-record";
...
const handleCommand = (command: number) => {
  ...
  if (command === 2) {
    // 2是全部导出
    let { clean_date, ...rest } = formData.value;
    let data = {
      clean_date_start: isArray(clean_date) ? clean_date[0] : "",
      clean_date_end: isArray(clean_date) ? clean_date[1] : "",
      ...rest,
    };
    startDownloadUrl(rinseRecordReportApi, data);
  } else {
    if (ids.value.length === 0) {
      return ElMessage.warning("请您至少勾选一条数据");
    }
    // 1是选择导出
    startDownloadUrl(rinseRecordReportApi, { id: ids.value });
  }
};
...
async function getData() {
  ...
  const result = await getRinseRecordListApi(data);
...
function handleAdd() {
  ...
  nextTick(() => {
    ...
    addFormData.value.clean_user_id = useUser.uid; //新建默认当前用户id
    addFormData.value.clean_user_name = useUser.nickname; //新建默认当前用户名称
    addFormData.value.remark = "";
    addFormData.value.clean_uv = "";
    addFormData.value.clean_date = dayjs().format("YYYY-MM-DD");
  });
}
...
function cellEdit(row: any) {
  ...
  addFormData.value.clean_user_id = row.clean_user_id;
  addFormData.value.clean_user_name = row.clean_user_name;
  addFormData.value.remark = row.remark;
  addFormData.value.clean_uv = row.clean_uv;
  addFormData.value.clean_date = row.clean_date;
  addVisible.value = true;
}
...
async function sendData(status: number = 0) {
  ...
  if (status === 1) {
    ...
    result = listId.value
      ? await rinseRecordConfirmApi({ ...data, id: listId.value })
      : await rinseRecordAddApi(data);
  } else {
    // 如果点击的保存,则判断是否有id,有则则调编辑接口,无则调新建接口
    result = listId.value
      ? await rinseRecordEditApi({ ...data, id: listId.value })
      : await rinseRecordAddApi(data);
  }
  confirm_sign.value = "";
...
function cellDel(row: any) {
  ElMessageBox.confirm(`确认要删除单据编号为：【${row.order_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await rinseRecordDelApi({ id: row.id });
      ElMessage.success(result.msg);
      getData();
    })
    .catch((error) => {
      ...
      <PureTableBar :columns="columns" @refresh="handleSearch">
        <template #buttons="scope">
          <el-button
            ...
            @click="handleAdd"
            :icon="Plus"
            v-hasPerm="['inst:rinserecord:add']"
          >
            新建
          </el-button>
          <ExportDownVue
            @handleCommand="handleCommand"
            v-hasPerm="['inst:rinserecord:report']"
          ></ExportDownVue>
        </template>
            ...
            <template #operation="{ row }">
              <template v-if="row.status === 0">
                <el-button
                  ...
                  link
                  @click="cellEdit(row)"
                  v-hasPerm="['inst:rinserecord:edit']"
                >
                  编辑
                </el-button>
                <!-- 当前是创建人的时候 -->
                <template v-if="checkAssocType(row.assoc_type, 1)">
                  <el-button
                    ...
                    link
                    @click="cellDel(row)"
                    v-hasPerm="['inst:rinserecord:del']"
                  >
                    删除
                  </el-button>
                </template>
    ...
    >
      <template #form-footer="{ handleSubmit }">
        ...
        <el-button
          ...
          v-hasPerm="['inst:rinserecord:confirm']"
        >
          签字确认
        </el-button>
      </template>
