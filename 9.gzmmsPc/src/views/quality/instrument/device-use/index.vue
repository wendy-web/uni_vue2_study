...
import {
  ...
  deviceUseReportApi,
  getDeviceUseListApi,
} from "@/api/quality/instrument/device-use";
...
const plusDialogTitle = ref("新增仪器使用记录");
const listId = ref(0);
...
const handleCommand = (command: number) => {
  ...
  if (command === 2) {
    ...
    startDownloadUrl(deviceUseReportApi, data);
  } else {
    // 1是选择导出
    ...
    if (allSameYear) {
      startDownloadUrl(deviceUseReportApi, { id: ids.value, use_year: selectYearList.value[0] });
    } else {
...
async function getData() {
  ...
  const result = await getDeviceUseListApi(data);
...
function handleAdd() {
  confirm_sign.value = "";
  listId.value = 0;
  plusDialogTitle.value = "新增仪器使用记录";
...
function cellEdit(row: any) {
  confirm_sign.value = "";
  listId.value = row.id;
  plusDialogTitle.value = "修改仪器使用记录";
...
async function sendData(status: number = 0) {
  ...
  if (status === 1) {
    ...
    result = listId.value
      ? await deviceUseConfirmApi({ ...data, id: listId.value })
      : await deviceUseAddApi(data);
  } else {
    // 如果点击的保存,则判断是否有id,有则则调编辑接口,无则调新建接口
    result = listId.value
      ? await deviceUseEditApi({ ...data, id: listId.value })
      : await deviceUseAddApi(data);
  }
  confirm_sign.value = "";
...
function cellDel(row: any) {
  ElMessageBox.confirm(`确认要删除单据编号为：【${row.order_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await deviceUseDelApi({ id: row.id });
      ElMessage.success(result.msg);
      getData();
    })
    .catch((error) => {
      ...
      <PureTableBar :columns="columns" @refresh="handleSearch">
        <template #buttons="scope">
          <el-button
            ...
            @click="handleAdd"
            :icon="Plus"
            v-hasPerm="['inst:deviceuse:add']"
          >
            新建
          </el-button>
          <ExportDownVue
            @handleCommand="handleCommand"
            v-hasPerm="['inst:deviceuse:report']"
          ></ExportDownVue>
        </template>
      ...
      <template #form-footer="{ handleSubmit }">
        <div class="mt-8">
          ...
          <el-button
            ...
            v-hasPerm="['inst:deviceuse:confirm']"
          >
            签字确认
          </el-button>
        </div>
