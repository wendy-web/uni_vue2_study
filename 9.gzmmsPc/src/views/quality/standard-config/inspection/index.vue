...
import {
  ...
  editApi,
  getListApi,
} from "@/api/quality/standard-config/inspection/index";
...
function handleAdd() {
  ...
  addFormData.value.stand_type = "";
  addFormData.value.desp = "";
  addFormData.value.name = "";
  dialogTitle.value = "新增检验依据";
...
const cellEdit = (row: any) => {
  ...
  addFormRef.value?.resetFields();
  dialogTitle.value = "编辑检验依据";
  listId.value = row.id;
  addFormData.value.name = row.name;
  addFormData.value.desp = row.desp;
  addFormData.value.stand_type = row.stand_type;
  addFormData.value.is_open = row.is_open;
  addVisible.value = true;
};
// 单元格点击删除
const cellDel = (row: any) => {
  ElMessageBox.confirm(`确认要删除检验依据为：【${row.name}】的该条内容吗?`, "警告", {
    ...
    <div class="app-card">
      <PureTableBar :columns="columns" @refresh="handleSearch">
        <template #buttons="scope">
          <el-button type="primary" @click="handleAdd" :icon="Plus" v-hasPerm="['sc:inspection:add']">新建</el-button>
        </template>
        <template v-slot="{ size, dynamicColumns }">
          ...
          >
            <template #operation="{ row }">
              <el-button type="primary" link @click="cellEdit(row)" v-hasPerm="['sc:inspection:edit']">编辑</el-button>
              <el-button type="primary" link @click="cellDel(row)" v-hasPerm="['sc:inspection:del']">删除</el-button>
            </template>
          </pure-table>
        </template>
      </PureTableBar>
