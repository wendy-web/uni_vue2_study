...
import {
  ...
  editApi,
  getListApi,
} from "@/api/quality/standard-config/productionteam/index";
...
const cellDel = (row: any) => {
  ElMessageBox.confirm(`确认要删除生产班组为：【${row.name}】的该条内容吗?`, "警告", {
    ...
    <div class="app-card">
      <PureTableBar :columns="columns" @refresh="handleSearch">
        <template #buttons="scope">
          <el-button type="primary" @click="handleAdd" :icon="Plus" v-hasPerm="['sc:productionteam:add']">新建</el-button>
        </template>
        <template v-slot="{ size, dynamicColumns }">
          ...
          >
            <template #operation="{ row }">
              <el-button type="primary" link @click="cellEdit(row)" v-hasPerm="['sc:productionteam:edit']">编辑</el-button>
              <el-button type="primary" link @click="cellDel(row)" v-hasPerm="['sc:productionteam:del']">删除</el-button>
            </template>
          </pure-table>
        </template>
      </PureTableBar>
