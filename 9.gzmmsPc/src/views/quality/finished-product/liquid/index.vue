...
const handleRowClick = (row: any, column: Column) => {
  ...
  if (column.property === "order_no") {
    router.push({
      path: "/quality/finished-product/liquid/add",
      query: {
        pageType: 3,
        id: row.id,
...
async function getData() {
  let { pro_date_arr, check_date_arr, ...rest } = formData.value;
  ...
  let data = {
    ...
    pro_date_start: isArray(pro_date_arr) ? pro_date_arr[0] : "",
    pro_date_end: isArray(pro_date_arr) ? pro_date_arr[1] : "",
    ...rest,
  };
    ...
    <div class="app-card">
      <PureTableBar :columns="columns" @refresh="handleSearch">
        <template #buttons="scope">
          <el-button type="primary" @click="handleAdd" :icon="Plus" v-hasPerm="['fp:liquid:add']">
            新建
          </el-button>
