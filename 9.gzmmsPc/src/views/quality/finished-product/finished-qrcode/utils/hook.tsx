...
export function useList(search: () => void) {
  ...
  const { statusOptions, getStatusText, getTagType, getBasePassLabel, getWorkshopLabel } =
    useselectData();
  ...
  const columns: TableColumnList = [
    ...
    },
    {
      ...
      align: "center",
      cellRenderer: ({ row }) => {
        return getWorkshopLabel(row.workshop) || "--";
      },
    },
    {
      label: "线别",
      ...
      cellRenderer: ({ row }) => (
        <>
          <ElTag
            ...
            onClick={() => showApproveLog({ id: row.id, orderType: 16, orderStatus: row.status })}
          >
