...
import {
  ...
  getFinishedQrcodeListApi,
} from "@/api/quality/finished-product/finished-qrcode/index";
import type { FinishedQrcodeListType } from "@/api/quality/finished-product/finished-qrcode/types";
...
const tableData = ref<FinishedQrcodeListType[]>([]);
...
async function getData() {
  ...
  const result = await getFinishedQrcodeListApi(data);
...
function handleAdd() {
  router.push({
    path: "/quality/finished-product/finished-qrcode/add",
  });
}

/** 点击生成报告 */
async function cellGenerateReport(row: FinishedQrcodeListType) {
  startDownloadUrl(finishedQrcodeReportApi, { id: row.id });
}

/** 点击编辑 */
function cellEdit(row: FinishedQrcodeListType) {
  router.push({
    path: "/quality/finished-product/finished-qrcode/add",
    query: {
      id: row.id,
...
/** 点击删除 */
function cellDel(row: FinishedQrcodeListType) {
  ElMessageBox.confirm(`确认要删除单据编号为：【${row.order_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await finishedQrcodeDelApi({ id: row.id });
...
/** 点击撤回 */
async function cellRecall(row: FinishedQrcodeListType) {
  const result = await finishedQrcodeRecallApi({ id: row.id });
  ElMessage.success(result.msg);
  getData();
}
      ...
      <PureTableBar :columns="columns" @refresh="handleSearch">
        <template #buttons="scope">
          <el-button
            ...
            @click="handleAdd"
            :icon="Plus"
            v-hasPerm="['fp:finishedqrcode:add']"
          >
            新建
          </el-button>
