...
import {
  ...
  makeReportApi,
  revokeOrderApi,
} from "@/api/quality/finished-product/saccharic-acid/index";
...
const { formData, searchColumns, columns, pagination, getBaseData, BaseData } = useList(handleSearch);
// /** 监听表单的变化 */
...
const handleRowClick = (row: any, column: Column) => {
  ...
  if (column.property === "order_no") {
    router.push({
      path: "/quality/finished-product/saccharic-acid/add",
      query: {
        pageType: 3,
        id: row.id,
    ...
    <div class="app-card">
      <PureTableBar :columns="columns" @refresh="handleSearch">
        <template #buttons="scope">
          <el-button type="primary" @click="handleAdd" :icon="Plus" v-hasPerm="['fp:saccharicacid:add']">新建</el-button>
