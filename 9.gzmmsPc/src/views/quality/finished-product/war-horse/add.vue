...
import {
  getProductWarhorseDetailApi,
  productWarhorseAddApi,
  productWarhorseApproveApi,
  productWarhorseDelApi,
  productWarhorseEditApi,
  productWarhorseRecallApi,
  productWarhorseRejectApi,
  productWarhorseReportApi,
  productWarhorseReverseApi,
  productWarhorseSubmitApi,
} from "@/api/quality/finished-product/war-horse";
...
import waitListVue from "@/views/quality/finished-product/redbull/components/waitList.vue";
...
function waitListChange(list: any[]) {
  ...
  const batchNumList = Object.values(groupsData.value).flat();

  checkInfoRef.value!.setMeanValue(batchNumList);
}
...
function handleCancel() {
  router.replace({
    path: "/quality/finished-product/war-horse",
  });
}
...
function handleDel() {
  ...
  ElMessageBox.confirm(`确认要删除单据编号为：【${order_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await productWarhorseDelApi({ id: listId.value });
...
const handleRecheckSignConfirm = async (fileValues: {
  ...
  const result =
    fileValues.status == 3
      ? await productWarhorseRejectApi(data)
      : await productWarhorseApproveApi(data);
  closeCurrentPage(result.msg);
};

/** 点击撤回 */
async function hanleRecall() {
  const result = await productWarhorseRecallApi({ id: listId.value });
  ElMessage.success(result.msg);
  getDetailData();
}
...
function handleReverse() {
  ElMessageBox.confirm(`确认要反审核该单据吗?`, "警告", {
    ...
    .then(async () => {
      const result = await productWarhorseReverseApi({ id: listId.value });
      ElMessage.success(result.msg);
      getDetailData();
    })
    .catch((error) => {
...
async function handleReport() {
  startDownloadUrl(productWarhorseReportApi, { id: listId.value });
}
...
async function handleSave(type = 1) {
  ...
  let check_info = checkInfoRef.value!.check_info; //检验结果
  let signature = checkInfoRef.value!.signature; //签字图片
  let conclusion = checkInfoRef.value!.conclusion; //检验意见
  let measureVal = checkInfoRef.value!.measureVal; // 测定值
  let data = {
    ...rest,
  ...
  try {
    const result = listId.value
      ? await productWarhorseEditApi({ ...data, id: listId.value })
      : await productWarhorseAddApi(data);
    ...
    if (type === 2) {
      ...
      const submitResult = await productWarhorseSubmitApi(data);
...
async function getDetailData() {
  ...
  const result = await getProductWarhorseDetailApi({ id: listId.value });
  ...
  console.log("🚀 ~ getDetailData ~  groupsData.value :", groupsData.value);
  tableData.value = formatTable(groupsData.value);

  checkInfoRef.value!.setData(res);
  ...
  check_info.forEach((item) => {
    let arr = item.batch_detail.map((subItem: any) => {
      return {
        ...
        id: subItem.id, //记录id
        phys_net_val: subItem.phys_net_val,
        phys_internal_pressure_val: subItem.phys_internal_pressure_val,
        ph_val: subItem.ph_val,
        soluble_solids_val: subItem.soluble_solids_val,
      };
    });
...
watch(
  ...
  (newVal) => {
    ...
    baseForm.value.lineName = newVal.map((item) => item.line).join(",");
  },
);
      ...
      <el-tab-pane label="单据内容" name="first">
        <el-collapse v-model="activeNames" class="mt-2">
          ...
          <el-collapse-item name="3" class="mt-2">
            ...
            <div class="px-8">
              <checkInfoVue :disabled="isDetailDisable" ref="checkInfoRef"></checkInfoVue>
            </div>
          </el-collapse-item>
