...
import {
  getProductRedbullListApi,
  productRedbullDelApi,
  productRedbullRecallApi,
  productRedbullReportApi,
} from "@/api/quality/finished-product/redbull/index";
import type { RedbullListType } from "@/api/quality/finished-product/redbull/types";
...
const tableData = ref<RedbullListType[]>([]);
...
async function getData() {
  ...
  const result = await getProductRedbullListApi(data);
...
function handleAdd() {
  router.push({
    path: "/quality/finished-product/redbull/add",
  });
}

/** 点击生成报告 */
async function cellGenerateReport(row: any) {
  startDownloadUrl(productRedbullReportApi, { id: row.id });
}

/** 点击编辑 */
function cellEdit(row: RedbullListType) {
  router.push({
    path: "/quality/finished-product/redbull/add",
    query: {
      id: row.id,
...
/** 点击删除 */
function cellDel(row: RedbullListType) {
  ElMessageBox.confirm(`确认要删除单据编号为：【${row.order_no}】的该条内容吗?`, "警告", {
    ...
    .then(async () => {
      ...
      const result = await productRedbullDelApi({ id: row.id });
...
/** 点击撤回 */
async function cellRecall(row: RedbullListType) {
  const result = await productRedbullRecallApi({ id: row.id });
  ElMessage.success(result.msg);
  getData();
}
    ...
    <div class="app-card">
      <PureTableBar :columns="columns" @refresh="handleSearch">
        <template #buttons="scope">
          <el-button type="primary" @click="handleAdd" :icon="Plus" v-hasPerm="['fp:redbull:add']">
            新建
          </el-button>
