...
export default {
  beforeRouteEnter(to, from, next) {
    let permsRes = perms(["buy:refund:add","buy:refund:edit"]);
    if (permsRes) {
      next((vm) => {});
    } else {
...
// 引入类型
import { IRefundPreInfo, IAddEmit } from "@/api/buy/refund/types";
...
const state = reactive({
  ...
  preTableData: {} as IRefundPreInfo, //传递给pre页面的数据
...
<template>
  <transition name="fade-transform" mode="out-in">
    <KeepAlive :include="['refundOrderAdd']">
      <component
        :is="comName"
