...
export default {
  beforeRouteEnter(to, from, next) {
    let permsRes = perms(["buy:order:add", "buy:order:edit"]);
    if (permsRes) {
      next((vm) => {});
    } else {
...
// 引入类型
import { IOrderPreInfo, IAddEmit } from "@/api/buy/order/types";
...
const state = reactive({
  ...
  preTableData: {} as IOrderPreInfo, //传递给pre页面的数据
