...
export default {
  beforeRouteEnter(to, from, next) {
    let permsRes = perms(["buy:swap:add","buy:swap:edit"]);
    if (permsRes) {
      next((vm) => {});
    } else {
...
/* 该页面动态切换add/edit页面组件和预览页面组件 */
import { useRouter, useRoute } from "vue-router";
import { useTagsViewStore } from "@/store/modules/tagsView";
// 引入添加/编辑页面组件 和 预览页面组件
import Add from "./components/add.vue";
import Preview from "./components/preview.vue";

import { IAddEmit } from "./utils/types";
...
const comType = ref(1); //1是add新建/编辑 2是预览页
const preTableData = ref();
const listId = ref(0); //采购单id 传给详情页,add编辑页
const editFrom = ref(0); // 从哪个组件进入add编辑页的标识, 1是从list组件过去,2是detail组件过去, 0是pre组件返回过去的
...
// 监听add页面的事件
const handleAddChange = (query: IAddEmit) => {
...
<template>
  <transition name="fade-transform" mode="out-in">
    <KeepAlive :include="['swapOrderAdd']">
      <component
        :is="comName"
