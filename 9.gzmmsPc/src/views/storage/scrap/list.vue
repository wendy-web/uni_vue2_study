...
//引入类型
import { IScrapItem } from "@/api/storage/scrap/types";
...
// 点击查看审批日志
async function showApproveLog(row: IScrapItem) {
  let { id, status, wh_scr_no } = row;
...
const getData = async () => {
  ...
  try {
    ...
    const result = await getScrapListApi(data);
...
// 单元格点击提审
const cellSubmitAudit = async (row: IScrapItem) => {
  console.log("点击提审");
  try {
    const result = await submitScrapApi({ id: row.id });
...
// 单元格点击撤回
const cellRecall = async (row: IScrapItem) => {
  try {
    const result = await recallScrapApi({ id: row.id });
    ElMessage.success(result.msg);
    getData();
  } catch (error) {}
};
// 单元格点击作废
const handleVoid = (row: IScrapItem) => {
  console.log("点击作废", row.id);
  ElMessageBox.confirm(`您确定要作废【${row.wh_scr_no}】报废单吗?`, "温馨提示", {
    ...
    .then(async () => {
      console.log("点击了 确定");
      try {
        let result = await voidScrapApi({ id: row.id });
        ElMessage.success(result.msg);
        getData();
      } catch (error) {}
    })
...
// 单元格点击删除
const cellDel = (row: IScrapItem) => {
  ElMessageBox.confirm(`您确定要删除【${row.wh_scr_no}】报废单吗?`, "温馨提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  })
    .then(async () => {
      console.log("点击了 确定");
      let result = await delScrapApi({ id: row.id });
      ElMessage.success(result.msg);
...
// 单元格点击通过
const cellApprove = async (row: IScrapItem) => {
  console.log("点击通过", row.id);
  try {
    const result = await approveScrapApi({ id: row.id });
    ElMessage.success(result.msg);
    getData();
  } catch (error) {}
};
...
// 单元格点击驳回
const cellReject = (row: IScrapItem) => {
