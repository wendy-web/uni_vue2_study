...
// 引入类型
import { ICheckItem } from "@/api/storage/check/types";
...
// 点击查看审批日志
async function showApproveLog(row: ICheckItem) {
  let { id, status, wh_inv_no } = row;
  if (!status) {
...
// 单元格点击导出
const cellExport = async (row: any) => {
  const result = await exportCheckApi(
    {
      id: row.id,
    },
    "blob",
  ...
  //拆解获取文件名，如果后端有给返回文件名的话
  let _fileName = result.headers["content-disposition"].split(";")[1].split("=")[1];
...
// 单元格点击提审
const cellSubmitAudit = async (row: ICheckItem) => {
  console.log("点击提审");
...
const sendSubmit = async (rowId: number, up_inv: number) => {
  try {
    const result = await submitCheckApi({ id: rowId, up_inv });
...
// 单元格点击撤回
const cellRecall = async (row: ICheckItem) => {
  try {
    const result = await recallCheckApi({ id: row.id });
    ElMessage.success(result.msg);
    getData();
  } catch (error) {}
};
// 单元格点击作废
const handleVoid = (row: ICheckItem) => {
  console.log("点击作废", row.id);
  ElMessageBox.confirm(`您确定要作废【${row.wh_inv_no}】盘点单吗?`, "温馨提示", {
    ...
    .then(async () => {
      console.log("点击了 确定");
      try {
        let result = await voidCheckApi({ id: row.id });
        ElMessage.success(result.msg);
        getData();
      } catch (error) {}
    })
...
// 单元格点击删除
const cellDel = (row: ICheckItem) => {
  ElMessageBox.confirm(`您确定要删除【${row.wh_inv_no}】盘点单吗?`, "温馨提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  })
    .then(async () => {
      console.log("点击了 确定");
      let result = await delCheckApi({ id: row.id });
      ElMessage.success(result.msg);
...
// 单元格点击通过
const cellApprove = async (row: ICheckItem) => {
  console.log("点击通过", row.id);
  try {
    const result = await approveCheckApi({ id: row.id });
    ElMessage.success(result.msg);
    getData();
  } catch (error) {}
};
...
// 单元格点击驳回
const cellReject = (row: ICheckItem) => {
          ...
          >
            ...
            <template #operation="{ row, $index }">
              ...
              <el-button
                size="default"
                link
                @click="cellExport(row)"
                v-hasPerm="['sto:check:export']"
              >
                导出
              </el-button>
              <!-- 当前为创建人 -->
