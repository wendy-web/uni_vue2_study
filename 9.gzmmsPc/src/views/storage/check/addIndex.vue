...
export default {
  beforeRouteEnter(to, from, next) {
    let permsRes = perms(["sto:check:add","sto:check:edit"]);
    if (permsRes) {
      next((vm) => {});
    } else {
...
const state = reactive({
  ...
  preTableData: {} as ICheckAddInfo, //传递给pre页面的数据
...
// 监听add页面的事件
const handleAddChange = (query: IAddEmit<ICheckAddInfo>) => {
...
<template>
  <transition name="fade-transform" mode="out-in">
    <KeepAlive :include="['StoCheckAdd']">
      <component
        :is="comName"
        @aboutAdd="handleAddChange"
        @aboutPre="handlePreChange"
        :storageList="storageFilterList"
