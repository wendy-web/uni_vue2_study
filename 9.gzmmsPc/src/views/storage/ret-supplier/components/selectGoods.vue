...
import { importRetSupInApi } from "@/api/storage/ret-supplier";
import type { IRetSupInImportItem } from "@/api/storage/ret-supplier/types";
...
const tableData = ref<IRetSupInImportItem[]>([]);
const total = ref(1);
const selectData = ref<IRetSupInImportItem[]>([]); //勾选的数据列表
...
const getData = async (order_no?: string) => {
  try {
    formData.value.wh_rec_no = order_no ? order_no : formData.value.wh_rec_no;
    const result = await importRetSupInApi(formData.value);
    if (result.code === "0") {
...
function changeSelect(selection: IRetSupInImportItem[]) {
