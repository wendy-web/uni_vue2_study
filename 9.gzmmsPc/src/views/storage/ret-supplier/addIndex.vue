...
export default {
  beforeRouteEnter(to, from, next) {
    let permsRes = perms(["sto:retsupin:add","sto:retsupin:edit"]);
    if (permsRes) {
      next((vm) => {});
    } else {
...
const state = reactive({
  ...
  preTableData: {} as IRetSupInAddInfo, //传递给pre页面的数据
...
// 监听add页面的事件
const handleAddChange = (query: IAddEmit<IRetSupInAddInfo>) => {
...
<template>
  <transition name="fade-transform" mode="out-in">
    <KeepAlive :include="['StoRetSupInAdd']">
      <component
        :is="comName"
