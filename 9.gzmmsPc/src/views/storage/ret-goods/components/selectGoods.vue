...
import { getImportStockApi } from "@/api/common/index";
import { GetImportList } from "@/api/common/types";
...
const tableData = ref<GetImportList[]>([]);
const total = ref(1);
const selectData = ref<GetImportList[]>([]); //勾选的数据列表
...
const getData = async (data?: { procure_no: string; warehouse_id: number }) => {
  try {
    if (data) {
      formData.value.procure_no = data.procure_no;
      formData.value.warehouse_id = data.warehouse_id;
    }
    const result = await getImportStockApi(formData.value);
    if (result.code === "0") {
...
function changeSelect(selection: GetImportList[]) {
...
defineExpose({ getData, setStatus, clearTable });
watch(
  () => props.ids,
  (newValue) => {
