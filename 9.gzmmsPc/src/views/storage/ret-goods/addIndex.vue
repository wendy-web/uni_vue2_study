...
export default {
  beforeRouteEnter(to, from, next) {
    let permsRes = perms(["sto:retgoods:add","sto:retgoods:edit"]);
    if (permsRes) {
      next((vm) => {});
    } else {
...
const state = reactive({
  ...
  preTableData: {} as IRetGoodsAddInfo, //传递给pre页面的数据
  listId: 0, //采购单id 传给详情页,add编辑页
  editFrom: 0, // 从哪个组件进入add编辑页的标识, 1是从list组件过去,2是detail组件过去, 0是pre组件返回过去的
  procure_no: "", //采购单号
});

const { comType, preTableData, listId, editFrom, procure_no } = toRefs(state);
...
procure_no.value = route.query.procure_no as string;

// 监听add页面的事件
const handleAddChange = (query: IAddEmit<IRetGoodsAddInfo>) => {
...
<template>
  <transition name="fade-transform" mode="out-in">
    <KeepAlive :include="['StoRetGoodsAdd']">
      <component
        :is="comName"
