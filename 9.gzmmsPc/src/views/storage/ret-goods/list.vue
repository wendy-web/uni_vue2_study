...
// 导入类型
import { IRetGoodsItem } from "@/api/storage/ret-goods/types";
...
// 点击查看审批日志
async function showApproveLog(row: IRetGoodsItem) {
  let { id, status, wh_ret_no } = row;
  if (!status) {
...
// 单元格点击编辑
const cellEdit = (row: IRetGoodsItem) => {
  router.push({
    path: "/storage/ret-goods/add",
    query: {
      editFrom: 1,
...
// 单元格点击详情
const cellDetail = (row: IRetGoodsItem) => {
  console.log(row.id);
  router.push({
    path: "/storage/ret-goods/detail",
    query: {
...
// 单元格点击提审
const cellSubmitAudit = async (row: IRetGoodsItem) => {
  console.log("点击提审");
  try {
    const result = await submitRetGoodsApi({ id: row.id });
...
// 单元格点击撤回
const cellRecall = async (row: IRetGoodsItem) => {
  try {
    const result = await recallRetGoodsApi({ id: row.id });
    ElMessage.success(result.msg);
    getData();
  } catch (error) {}
};
// 单元格点击作废
const handleVoid = (row: IRetGoodsItem) => {
  console.log("点击作废", row.id);
  ElMessageBox.confirm(`您确定要作废【${row.wh_ret_no}】其他出库单吗?`, "温馨提示", {
    ...
    .then(async () => {
      console.log("点击了 确定");
      try {
        let result = await voidRetGoodsApi({ id: row.id });
        ElMessage.success(result.msg);
        getData();
      } catch (error) {}
    })
...
// 单元格点击删除
const cellDel = (row: IRetGoodsItem) => {
  ElMessageBox.confirm(`您确定要删除【${row.wh_ret_no}】其他出库单吗?`, "温馨提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  })
    .then(async () => {
      console.log("点击了 确定");
      let result = await delRetGoodsApi({ id: row.id });
      ElMessage.success(result.msg);
...
// 单元格点击通过
const cellApprove = async (row: IRetGoodsItem) => {
  console.log("点击通过", row.id);
  try {
    const result = await approveRetGoodsApi({ id: row.id });
    ElMessage.success(result.msg);
    getData();
  } catch (error) {}
};
...
// 单元格点击驳回
const cellReject = (row: IRetGoodsItem) => {
    ...
    <div class="search-card">
      <el-form :model="formData" ref="form" :inline="true">
        <el-form-item label="关键字" prop="keyword" class="w-[246px]">
          <el-input v-model="formData.keyword" placeholder="出库单号/退货单号/制单人"></el-input>
        </el-form-item>
