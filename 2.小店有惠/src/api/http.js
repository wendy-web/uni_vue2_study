...
const whiteList = ["/api/post/wxlogin"];
//默认请求头
const defaultHeader = {
...
function _res(response, config) {
    ...
    if (response.statusCode == 200) {
        ...
        if (result.code == -1 || (result.code == 0 && isLoginURL) || (config.data && config.data.hasOwnProperty("unionid") && !config.data.unionid)) {
            //确保没有其它请求干扰了
...
XHService.prototype._request = function(config) {
    ...
    config.header = {};
    /*平台*/
    config.header["device"] = platform || wx.getSystemInfoSync().platform;
    ...
    return new Promise((resolve, reject) => {
        ...
        if (config.isCache) {
            ...
            if (
                cache &&
                VALID_CACHE(config.cacheMaxAge || _cacheTime, cache.lastModified)
            ) {
                if (REQLISTSIZE.length > 0) {
                    REQLISTSIZE.pop();
                }
...
function getHttpErrMsg(errMsg) {
    return `亲，您的网络好像出现了状况，请稍后再尝试哟！(${errMsg.replace(
    /(request:fail)|(。)/g,
    ""
  )})`;
}
