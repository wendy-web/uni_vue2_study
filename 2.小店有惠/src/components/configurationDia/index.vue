...
>
    <view class="dia_cont-box ani_active">
        ...
        <view class="dia_com" v-else-if="currentPageNum == 4 && !config.image">
          ...
          <view class="card_cont" @click="popoverRember">
            <view class="card_lab-top">
              <van-icon name="fire" color="#EF2B20" style="margin-right: 8rpx;" />
              近24小时成交<text style="color: #EF2B20;margin: 0 8rpx;">{{config.cjNumber}}</text>件
            </view>
            ...
            <view class="item_title txt_ov_ell2">
              <view class="item_title-value">
                  {{ config.lx_type == 2 ? '京东' : '拼多多' }}
              </view>
              {{ config.skuName }}
            </view>
            <view class="card_price">
              <view class="card_price-txt">{{ config.face_value ? '券后' : '到手价'}}</view>
              <view v-html="formatPrice(config.coupon_price || 0)"></view>
              <view class="card_price-dis" v-if="config.face_value">¥{{ config.mark_price }}</view>
            </view>
            <view class="card_btn">
              推广立赚 <text class="card_btn-price">{{ config.rebateMoney }}</text>
            </view>
          </view>
        </view>
        <!-- 京东商品 - 无图片的状态 -->
